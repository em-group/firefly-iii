...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Hvis du kjører din egen installasjon av '.config('app.name').', kan du sjekke loggene i <code>storage/logs</code> for instruksjoner.',
    ...
    'need_more_help'                             => 'Hvis du trenger mer hjelp med å bruke '.config('app.name').', vennligst <a href="https://github.com/firefly-iii/firefly-iii/issues">opprett en sak på Github</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' kan ikke videresende deg til riktig side. Beklager.',
    ...
    'invalid_locale_settings'                    => config('app.name').' kan ikke formatere pengebeløp fordi serveren din mangler de nødvendige pakkene. Se følgende <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">instruksjoner for hvordan du setter opp dette</a>.',
    ...
    'login_page_title'                           => 'Innlogging til '.config('app.name'),
    'register_page_title'                        => 'Registrer deg på '.config('app.name'),
    'forgot_pw_page_title'                       => 'Glemt passord for '.config('app.name'),
    'reset_pw_page_title'                        => 'Nullstill passord for '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version autorisasjonsforespørsel',
    ...
    'admin_update_check_explain'                 => config('app.name').' kan sjekke om oppdateringer automatisk. Når du aktiverer denne innstillingen, vil systemet kontakte Github for å se om en ny versjon av '.config('app.name').' er tilgjengelig. Når det er kommet en ny oppdatering, vil du bli varslet. Du kan teste dette varselet ved hjelp av knappen til høyre. Vennligst marker nedenfor hvis du vil at '.config('app.name').' skal se etter oppdateringer.',
    'check_for_updates_permission'               => config('app.name').' kan se etter oppdateringer, men det trenger din tillatelse til å gjøre det. Gå til <a href=":link">-administrasjonen</a> for å velge om denne funksjonen skal aktiveres.',
    ...
    'admin_update_check_now_explain'             => 'Hvis du trykker på knappen, vil '.config('app.name').' se om din nåværende versjon er den nyeste.',
    'check_for_updates_button'                   => 'Sjekk nå!',
    'update_new_version_alert'                   => 'En ny versjon av '.config('app.name').' er tilgjengelig. Du kjører v:your_version, den nyeste versjonen er v:new_version som ble utgitt den :date.',
    ...
    'search_found_transactions'                  => config('app.name').' fant :count transaksjoner i :time sekunder.',
    'search_for_query'                           => config('app.name').' leter etter transaksjoner med disse ordene: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Søk',
    'search_box_intro'                           => 'Velkommen til søkefunksjonen i '.config('app.name').'. Skriv inn søkefrasen din i boksen. Pass på at du sjekker hjelpefilen, for søkefunksjonen er ganske avansert.',
    ...
    'instructions_rule_from_bill'                => 'For å matche transaksjoner til din nye regning ":name", kan '.config('app.name').' lage en regel som automatisk vil bli sjekket mot eksisterende transaksjoner. Vennligst verifiser detaljene nedenfor og lagre regelen for å få '.config('app.name').' til å automatisk matche transaksjoner til din nye regning.',
    ...
    'apply_rule_selection_intro'                 => 'Regler som ":title" brukes normalt bare til nye eller oppdaterte transaksjoner, men du kan få '.config('app.name').' til å kjøre dem på et utvalg av dine eksisterende transaksjoner. Dette kan være nyttig når du har oppdatert en regel, og du trenger at endringene blir brukt på alle dine tidligere transaksjoner.',
    ...
    'apply_rule_group_selection_intro'           => 'Regelgrupper som ":title" brukes normalt bare til nye eller oppdaterte transaksjoner, men du kan få '.config('app.name').' til å kjøre dem på et utvalg av dine eksisterende transaksjoner. Dette kan være nyttig når du har oppdatert en regelgruppe, og du trenger at endringene blir brukt på alle dine tidligere transaksjoner.',
    ...
    'create_rule_for_bill_txt'                => 'Du har nettopp opprettet en ny regning kalt ":name", gratulerer! '.config('app.name').' kan automagisk matche nye utbetalinger til denne regningen. Når du betaler husleien, vil for eksempel regningen "husleie" knyttes til utbetalingen. På denne måten kan '.config('app.name').' nøyaktig vise hvilke regninger som ligger til forfall. For å gjøre dette, må du opprette en ny regel. '.config('app.name').' har fylt ut noen fornuftig standard verdier for deg. Kontroller at disse er riktige. Hvis disse verdiene er riktige vil '.config('app.name').' automatisk koble riktig utbetaling til riktig regning. Sjekk ut triggerne for å se om de er riktige, og legg til noen hvis de er feil.',
    ...
    'pref_languages_help'                     => config('app.name').' støtter flere språk. Hvilket foretrekker du?',
    ...
    'delete_your_account_help'                => 'Hvis du sletter brukerkontoen din, slettes også alle kontoer, transaksjoner, <em>og alt annet</em> du har lagret i '.config('app.name').'. Det blir BORTE.',
    ...
    'secure_pw_ff'                            => 'Bruker du det samme passordet over alt på Internett? Hvis bare ett nettsted mister passordet, har hackere tilgang til alle dataene dine. '.config('app.name').' er avhengig av at du velger et sterkt og unikt passord for å beskytte dine finansielle data.',
    'secure_pw_check_box'                     => 'For å hjelpe deg kan '.config('app.name').' sjekke om passordet du vil bruke har blitt stjålet tidligere. Hvis dette er tilfelle, råder '.config('app.name').' deg til ikke å bruke dette passordet.',
    'secure_pw_working_title'                 => 'Hvordan virker det?',
    'secure_pw_working'                       => 'Ved å huke av denne boksen vil '.config('app.name').' sende de fem første tegnene til SHA1-hashen av passordet ditt til <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">nettsiden til Troy Hunt</a> for å se om det er på listen hans. Dette vil stoppe deg fra å bruke usikre passord som anbefalt i den siste <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">spesialpublikasjonen fra NIST</a> om dette emnet.',
    ...
    'email_verification'                      => 'En e-postmelding sendes til din gamle og nye e-postadresse. Av sikkerhetsgrunner, vil du ikke kunne logge inn før du bekrefter din nye epostadresse. Hvis du er usikker på om '.config('app.name').' installasjonen er i stand til å sende e-post, ikke bruk denne funksjonen. Hvis du er administrator, kan du teste dette i <a href="/admin">Administrasjon</a>.',
    ...
    'delete_local_info_only'                  => 'Fordi du godkjenner gjennom ":login_provider", vil dette bare slette lokal informasjon i '.config('app.name').'.',
    ...
    'currencies_intro'                        => config('app.name').' støtter ulike valutaer som du kan definere og aktivere her.',
    ...
    'auto_match_on'                           => 'Automatisk funnet av '.config('app.name'),
    'auto_match_off'                          => 'Ikke automatisk funnet av '.config('app.name'),
    ...
    'reconcile_has_more'                      => 'Din '.config('app.name').'-hovedbok har mer penger i seg enn banken hevder du burde ha. Det finnes flere alternativer. Vennligst velg hva du skal gjøre. Deretter trykker du på "Bekreft avstemming".',
    'reconcile_has_less'                      => 'Din '.config('app.name').'-hovedbok har mindre penger i seg enn banken hevder du burde ha. Det finnes flere alternativer. Vennligst velg hva du skal gjøre. Deretter trykker du på "Bekreft avstemming".',
    'reconcile_is_equal'                      => 'Din '.config('app.name').'-hovedbok og dine kontoutskrifter samsvarer. Det er ingenting du trenger å gjøre. Vennligst trykk "Bekreft avstemming" for å fortsette.',
    ...
    // new user:
    'welcome'                                 => 'Velkommen til '.config('app.name').'!',
    'submit'                                  => 'Send inn',
    'submit_yes_really'                       => 'Registrer (Jeg vet hva jeg gjør)',
    'getting_started'                         => 'Kom igang',
    'to_get_started'                          => 'Det er godt å se at du har installert '.config('app.name').'. Vennligst skriv inn bankens navn og saldo på hovedkontoen din for å komme i gang. Du trenger Ikke bekymre deg hvis du har flere kontoer. Du kan legge dem til senere. Dette er bare for at '.config('app.name').' skal få noe data å arbeide med.',
    'savings_balance_text'                    => config('app.name').' vil automatisk opprette en sparekonto for deg. Som standard vil det ikke være penger på sparekontoen din, men hvis du oppgir saldoen din til '.config('app.name').' så blir den lagt inn for deg.',
    'finish_up_new_user'                      => 'Det var alt! Du kan fortsette ved å trykke <strong>Send</strong>. Deretter vil du bli tatt til startskjermen til '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'Jippi! De nye kontoene dine ble lagret.',
    'set_preferred_language'                  => 'Hvis du foretrekker å bruke '.config('app.name').' med et annet språk, vennligst indiker det her.',
    'language'                                => 'Språk',
    ...
    'import_index_title'                    => 'Importer transaksjoner til '.config('app.name'),
    'import_data'                           => 'Importer data',
    'import_transactions'                   => 'Importer transaksjoner',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Denne funksjonen er ikke tilgjengelig når du bruker '.config('app.name').' i et Sandstorm.io miljø.',
    ...
    'no_accounts_imperative_asset'          => 'For å begynne å bruke '.config('app.name').' må du opprette minst én aktivakonto. La oss gjøre det nå:',
    ...
    'no_recurring_intro_default'            => 'Du har ingen gjentakende transaksjoner ennå. Du kan bruke disse for å få '.config('app.name').' til å automatisk opprette transaksjoner for deg.',
    ...
    'warning_duplicates_repetitions'        => 'I sjeldne tilfeller vises datoene to ganger i listen. Dette kan skje når flere gjentakelser kolliderer. '.config('app.name').' genererer alltid én transaksjon per dag.',
    ...
    'recurring_never_cron'                  => 'Det synes at cron jobben som er nødvendig for å støtte gjentakende transaksjoner har aldri kjørt. Dette er selvfølgelig normalt når du nettopp har installert '.config('app.name').', men dette bør settes opp så snart som mulig. Sjekk ut hjelp-sidene ved å bruke (?) -ikonet i øverste høyre hjørne på siden.',
    ...
    'help_first_date_no_past'            => 'Spesifiser den første forventede gjentakelse. '.config('app.name').' vil ikke opprette transaksjoner i fortiden.',
    ...
    'help_weekend'                       => 'Hva skal '.config('app.name').' gjøre når gjentakende transaksjon faller på en Lørdag eller Søndag?',
