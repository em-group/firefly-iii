...
return [
    ...
    'index_breadcrumb'                    => 'Importer data til '.config('app.name'),
    ...
    // index page:
    'general_index_intro'                 => 'Velkommen til '.config('app.name').'s importrutine. Det er flere måter å importere data på i '.config('app.name').', vist her som knapper.',
    ...
    'prereq_spectre_text'                 => 'For å importere data ved hjelp av Spectre API (v4), må du angi '.config('app.name').' to nøkkel verdier. De kan finnes på <a href="https://www.saltedge.com/clients/profile/secrets">secrets page</a>.',
    ...
    'prereq_bunq_ip'                      => 'bunq krever din eksterne (internett) IP-adresse. '.config('app.name').' har forsøkt å fylle dette ved å bruke <a href="https://www.ipify.org/">ipify service</a>. Kontroller at IP-adressen er riktig, hvis ikke vil importen mislykkes.',
    ...
    'callback_not_tls'                    => config('app.name').' har oppdaget følgende callback URI. Det ser ut som serveren din er ikke konfigurert til å godta TLS-tilkoblinger (https). YNAB vil ikke godta denne URI. Du kan fortsette med importen (fordi '.config('app.name').' kan ta feil), men vær obs.',
    ...
    'job_config_file_upload_text'         => 'Denne rutinen vil hjelpe deg med å importere filer fra banken din til '.config('app.name').'. ',
    'job_config_file_upload_help'         => 'Velg fil. Husk å kontroller at filen er UTF-8-kodet.',
    'job_config_file_upload_config_help'  => 'Hvis du tidligere har importert data til '.config('app.name').', kan du ha en konfigurasjonsfil liggende som har forhåndsfylte konfigurasjonsverdier for deg. For enkelte banker har andre brukere lagt ut konfigurasjonsfiler, se <a href="https://github.com/firefly-iii/import-configurations/wiki">konfigurasjonen fil</a>',
    ...
    'file_not_utf8'                       => 'Filen du har lastet opp er ikke kodet som UTF-8 eller ASCII. '.config('app.name').' kan ikke håndtere slike filer. Bruk Notepad ++ eller Sublime for å konvertere filen til UTF-8.',
    ...
    'job_config_uc_specifics_txt'         => 'Enkelte banker gir dårlig formaterte filer. '.config('app.name').' kan fikse disse automatisk. Hvis banken din leverer slike filer men er ikke oppført her, vennligst lag en sak på GitHub.',
    ...
    'job_config_spectre_login_text'       => config('app.name').' har funnet :count eksisterende innlogginger på din Spectre konto. Hvilken vil du importere fra?',
    ...
    'job_config_spectre_accounts_text'    => 'Du har valgt ":name" (:country). Du har :count tilgjengelige kontorer fra denne leverandøren. Velg '.config('app.name').' bruks/aktiva kontoen(e) der transaksjonene fra disse kontoene skal lagres. Husk, for å importere data fra '.config('app.name').' kontoen og ":name"-kontoen må begge ha samme valuta.',
    ...
    'job_config_ynab_select_budgets_text'  => 'Du har :count budsjett lagret hos YNAB. Vennligst velg en som '.config('app.name').' kan importere transaksjonene til.',
    ...
    'job_config_local_account_help'   => 'Velg '.config('app.name').' kontoen knyttet til din bank konto valgt ovenfor.',
    // specifics:
    ...
    'status_fatal_more'               => 'Denne (muligens meget krytpiske) feilmeldingen suppleres av loggfiler, som du finner på harddisken eller i Docker container der du kjører '.config('app.name').' fra.',
    ...
    'result_many_transactions'        => config('app.name').' har importert :count transaksjoner. De er lagret under tag <a href=":route" class="label label-success" style="font-size:100%;font-weight:normal;">:tag</a> hvor du kan undersøke dem videre.',
