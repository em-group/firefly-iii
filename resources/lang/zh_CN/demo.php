...
return [
    ...
    'index'                  => '欢迎来到 <strong>'.config('app.name').'</strong>！您可在此页快速概览您的财务状况。如需更多资， 请前往帐户 &rarr; <a href=":asset">资产帐户</a> 亦或是 <a href=":budgets">预算</a> 以及 <a href=":reports">报表</a> 页面。您也可以继续浏览此页。',
    ...
    'reports-index-start'    => config('app.name').' 支持数种不同的报表形式，您可以点选右上方的 <i class="fa fa-question-circle"></i>-图示获得更多资讯。',
    'reports-index-examples' => '请确认您以检阅过以下范例：<a href=":one">月财务概览</a>、<a href=":two">年度财务概览</a> 以及 <a href=":three">预算概览</a>。',
    'currencies-index'       => config('app.name').' 支持多种货币，即便预设为欧元，亦可设成美金或其他货币。如您所见，系统已包含了一小部分的货币种类，但您也可自行新增其他货币。修改预设货币并不会改变既有交易的货币种类，且 '.config('app.name').' 支持同时使用不同货币。',
    'transactions-index'     => '这些支出、储蓄与转帐并非蓄意虚构，而是自动产生的。',
    'piggy-banks-index'      => '如您所见，目前有3个小猪扑满。使用 + 号与 - 号按钮可改变每个小猪扑满的总额，而点选小猪扑满的名称则可管理该扑满。',
    'import-index'           => '任何 CSV 格式的档案都可导入 '.config('app.name').'，本程式也支持来自 bunq 与 Spectre 的档案格式，其他银行与金融机构则会在未来提供支持。而作为一名展示使用者，你只会看到「假的」供应者，系统会随机产生交易纪录以告知您如何运作。',
