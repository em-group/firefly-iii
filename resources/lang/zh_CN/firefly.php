...
return [
    ...
    'two_factor_lost_fix_self'                   => '若您自行营运 '.config('app.name').' 伺服器，请检察 <code>storage/logs</code> 内的日志以供指引。',
    ...
    'need_more_help'                             => '如果您需要更多 '.config('app.name').' 的协助，请 <a href="https://github.com/firefly-iii/firefly-iii/issues">于 GitHub 建立提问</a>。',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' 无法重新指向您至正确页面，抱歉。',
    ...
    'invalid_locale_settings'                    => config('app.name').' 无法格式化金额总数，因为您的伺服器缺乏必要套件。此处为 <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">如何处置的指引</a>。',
    'quickswitch'                                => '快速切换',
    ...
    'reset_button'                               => '重置',
    'want_to_login'                              => '我想登录',
    'login_page_title'                           => '登录到'.config('app.name'),
    'register_page_title'                        => '在 '.config('app.name').' 注册',
    'forgot_pw_page_title'                       => '忘记您 '.config('app.name').' 的密码',
    'reset_pw_page_title'                        => '重设您 '.config('app.name').' 的密码',
    ...
    'expected_total'                             => '期望总数',
    // API access
    'authorization_request'                      => config('app.name').' :version 版授权请求',
    ...
    'admin_update_check_explain'                 => config('app.name').' 可以自动检查更新。当启用此设定时，将会自动连接 Github 并查看是否有新版本可用，若有，您会收到一则通知。您可使用右侧的按钮测试通知功能，并于下方表示您想要 '.config('app.name').' 检查更新与否。',
    'check_for_updates_permission'               => config('app.name').' 可自动检查更新，但此功能需要您的许可。请转至 <a href=":link">后台管理</a> 表明您是否需要启用此功能。',
    'updates_ask_me_later'                       => '稍后再问我',
    'updates_do_not_check'                       => '不检查更新',
    'updates_enable_check'                       => '启用更新检查',
    'admin_update_check_now_title'               => '立刻检查更新',
    'admin_update_check_now_explain'             => '如果您按下此按钮，'.config('app.name').' 将会检查您现在的版本是否为最新版。',
    'check_for_updates_button'                   => '马上检查！',
    'update_new_version_alert'                   => '较新版本的 '.config('app.name').' 已可使用，您目前为 :your_version 版，最新版本为 :new_version 版，于 :date 释出。',
    ...
    'search_box'                                 => '搜寻',
    'search_box_intro'                           => '欢迎使用 '.config('app.name').' 的搜寻功能，请于方框内键入搜寻条件。请确保您以阅读过协助档案，因为此搜寻功能非常先进。',
    ...
    'export_data_intro'                          => '使用汇出资料转移至新的财务应用程式。请注意这些档案并非备份，它们未包含足够的后设资料以供新安装的 '.config('app.name').' 作为完全还原所用。如果您希望备份您的资料，请直接备份整个资料库。',
    'export_format'                              => '汇出格式',
    'export_format_csv'                          => '以逗号分隔的数值 (CSV档案)',
    'export_format_mt940'                        => 'MT940 相容格式',
    'include_old_uploads_help'                   => config('app.name').' 不会扔掉过去已导入的原始 CSV 档，你可以将它们包含在导出的档案中。',
    'do_export'                                  => '汇出',
    ...
    'instructions_rule_from_bill'                => '为了媒合交易至您的新帐单 ":name"，'.config('app.name').' 可以建立一个针对您储存的每一笔交易且自动启动的规则。请验证下方的资讯并储存规则，让 '.config('app.name').' 自动媒合交易至您的新帐单。',
    'rule_is_strict'                             => '严谨规则',
    ...
    'apply_rule_selection_intro'                 => '规则如 ":title" 一般仅套用至新的或更新后的交易，但您可要求 '.config('app.name').' 针对既有的单笔或多笔交易执行规则。在您更新一则规则后，且必须套用该规则至其他交易时，即可使用此功能。',
    ...
    'apply_rule_group_selection_intro'           => '规则群组如 ":title" 一般仅套用至新的或更新后的交易，但您可要求 '.config('app.name').' 针对既有的单笔或多笔交易执行规则群组内的规则。在您更新一则规则群组后，且必须套用该群组至其他交易时，即可使用此功能。',
    ...
    'create_rule_for_bill_txt'                => '恭喜，您方才建立了名为 ":name" 新帐单！'.config('app.name').' 可魔幻自动地媒合新提款至此帐单。举例而言，无论何时您付了房租，帐单 "房租" 会连结到该支出。依此，'.config('app.name').' 可以准确的显示哪些帐单即期又哪些尚未。若要使用此功能，必须建立一个新的规则。'.config('app.name').' 已为您填写一些合理的预设资料，请确认是否正确。若然，'.config('app.name').' 将自动连结正确的提款至正确的帐单，请检查触发器并确认无误，若有错误请另行增添。',
    ...
    'pref_1Y'                                 => '1年',
    'pref_languages'                          => '语言',
    'pref_languages_help'                     => config('app.name').' 支援多种语言，您倾向使用何者？',
    ...
    'delete_your_account_help'                => '删除您的帐户亦将删除任何帐户、交易、<em>任何</em> 您可能存再 '.config('app.name').' 的东西，均将消逝。',
    ...
    'secure_pw_ff'                            => '您在网路上都使用一样的密码吗？如果一个网站遗失您的密码，骇客将可通行您的所有资料。'.config('app.name').' 需要您选择强大且独特的密码以保护您的财务纪录。',
    'secure_pw_check_box'                     => '为了协助您达成安全密码的目的，'.config('app.name').' 可自动检查您的密码过去是否已被盗用。若然，'.config('app.name').' 建议您避免使用此密码。',
    'secure_pw_working_title'                 => '它是如何运作的？',
    'secure_pw_working'                       => '藉由勾选此方块，'.config('app.name').' 会传送您密码产生的SHA1 hash 的前5字元至 <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">特洛伊猎杀网站</a> 以检查是否上榜。如最新的 <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">NIST 特殊刊物</a> 与此相关的主题所建议的，结果将制止您使用不安全的密码。',
    ...
    'email_verification'                      => '一封电子邮件将发送到您的旧的和新的电子邮件地址。出于安全考量，在验证新的电子邮件地址之前，您将无法登入。如果您不确定您的 '.config('app.name').' 安装是否能够发送电子邮件，请不要使用此功能。如果您是管理员，可以在 <a href="/admin">后台管理</a> 中对此进行测试。',
    ...
    'delete_local_info_only'                  => '由于您以 ":login_provider" 验证，仅会删除本机 '.config('app.name').' 资讯。',
    ...
    'currencies_intro'                        => config('app.name').' 支援多种货币，您可在此设定并启用。',
    ...
    'auto_match_on'                           => '由 '.config('app.name').' 自动配对',
    'auto_match_off'                          => '由 '.config('app.name').' 不自动配对',
    ...
    'reconcile_has_more'                      => '您的 '.config('app.name').' 帐目比您的银行有更多的金钱，有多个选项可应处，请选择并按下 "确认对帐"。',
    'reconcile_has_less'                      => '您的 '.config('app.name').' 帐目比您的银行有更少的金钱，有多个选项可应处，请选择并按下 "确认对帐"。',
    'reconcile_is_equal'                      => '您的 '.config('app.name').' 帐目和您的银行相符，无须处理，请按下 "确认对帐" 确认导入。',
    ...
    // new user:
    'welcome'                                 => '欢迎使用 '.config('app.name').'！',
    'submit'                                  => '送出',
    'submit_yes_really'                       => '送出 (我知道我在干嘛)',
    'getting_started'                         => '开始使用',
    'to_get_started'                          => '很高兴看到您成功安装 '.config('app.name').'。要开始使用此工具，请输入您主要使用的的银行名称与余额。别担心，如您有多个帐户，可以稍后新增，这儘是为了 '.config('app.name').' 需要初始使用而设。',
    'savings_balance_text'                    => config('app.name').' 会自动为您建立储蓄帐户。预设下，储蓄帐户会没有金钱，但若您告诉 '.config('app.name').' 余额为何，亦可以此储存。',
    'finish_up_new_user'                      => '这就对了！您可按下 <strong>送出</strong> 继续，您将被带至 '.config('app.name').' 的首页。',
    'stored_new_accounts_new_user'            => '太好了！您的帐户已被储存。',
    'set_preferred_language'                  => '若您倾向在 '.config('app.name').' 使用其他语言，请于此表示。',
    'language'                                => '语系',
    ...
    'firefly_instance_configuration'        => config('app.name').' 的组态设定选项',
    ...
    'sandstorm_not_available'               => '当您在 Sandstorm.io 环境使用 '.config('app.name').' 时，此功能不可用。',

    // empty lists? no objects? instructions:
    'no_accounts_title_asset'               => '建立新资产帐户',
    'no_accounts_intro_asset'               => '您还没有资产帐户。资产帐户可以包括: 你的支票帐户、储蓄帐户、共用帐户甚至信用卡。',
    'no_accounts_imperative_asset'          => '要使用 '.config('app.name').' ，您至少需要一个资产账户。我们现在来创建它：',
    ...
    'no_recurring_intro_default'            => '您目前没有定期重复的交易。您可使用此功能让 '.config('app.name').' 自动为您建立交易。',
    ...
    'warning_duplicates_repetitions'        => '在少数情况下，日期在此清单上会显示两次，这在多个重复衝突之下会发生。'.config('app.name').' 每天只会产生一条纪录。',
    ...
    'recurring_never_cron'                  => '用来支援定期重复交易的 cron job 似乎没有运行过，这在您刚安装 '.config('app.name').' 没多久时是非常正常的，但能越快处理越好。请使用本页右上角的 (?)-图示查阅说明页面。',
    ...
    'help_first_date_no_past'            => '表示第一笔预期的周期交易，'.config('app.name').' 不会建立过去的交易。',
    'no_currency'                        => '(没有货币)',
    ...
    'help_weekend'                       => '当定期重复交易落于周六及周日时， '.config('app.name').' 该如何应处？',
    'do_nothing'                         => '就建立交易',
