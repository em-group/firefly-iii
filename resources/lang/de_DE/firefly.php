...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Wenn Sie Ihre eigene Instanz von '.config('app.name').' betreiben, überprüfen Sie die Logdatei unter <code>storage/logs</code> für weitere Anweisungen.',
    ...
    'need_more_help'                             => 'Wenn Sie Hilfe beim Bedienen von '.config('app.name').' brauchen, <a href="https://github.com/firefly-iii/firefly-iii/issues">erstellen Sie ein Ticket auf Github</a>.',
    ...
    'cannot_redirect_to_account'                 => 'Entschuldigen Sie bitte. '.config('app.name').' kann Sie nicht zur richtigen Seite weiterleiten.',
    ...
    'invalid_locale_settings'                    => config('app.name').' kann keine Geldbeträge formatieren, da auf Ihrem Server die erforderlichen Pakete fehlen. Es gibt <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">eine Anleitung</a>, wie dies behoben werden kann.',
    ...
    'login_page_title'                           => 'Bei '.config('app.name').' anmelden',
    'register_page_title'                        => 'Bei '.config('app.name').' registrieren',
    'forgot_pw_page_title'                       => 'Haben Sie Ihr Passwort für '.config('app.name').' vergessen?',
    'reset_pw_page_title'                        => 'Passwort für '.config('app.name').' zurücksetzen',
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version Autorisierungsanfrage',
    ...
    'admin_update_check_explain'                 => config('app.name').' kann automatisch nach Updates suchen. Wenn Sie diese Einstellung aktivieren, wird Github aufgerufen, um festzustellen, ob eine neue Version von '.config('app.name').' verfügbar ist. Falls dies der Fall ist, erhalten Sie eine Benachrichtigung. Sie können diese Benachrichtigung mit der Schaltfläche auf der rechten Seite testen. Bitte geben Sie unten an, ob Sie möchten, dass '.config('app.name').' nach Updates sucht.',
    'check_for_updates_permission'               => config('app.name').' kann nach Updates suchen, benötigt jedoch Ihre Erlaubnis dazu. Bitte gehen Sie zur <a href=":link">Administration</a>, um anzugeben, ob diese Funktion aktiviert werden soll.',
    ...
    'admin_update_check_now_explain'             => 'Wenn Sie den Knopf drücken, wird '.config('app.name').' überprüfen, ob Ihre Version die aktuellste ist.',
    'check_for_updates_button'                   => 'Jetzt prüfen!',
    'update_new_version_alert'                   => 'Eine neue Version von '.config('app.name').' ist verfügbar. Sie verwenden v:your_version, die neueste Version ist v:new_version und wurde am :date veröffentlicht.',
    ...
    'search_found_transactions'                  => config('app.name').' hat :count Buchung(en) in :time Sekunden gefunden.',
    'search_for_query'                           => config('app.name').' sucht nach Buchungen mit folgenden Wörtern im Namen: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Suche',
    'search_box_intro'                           => 'Willkommen zur Suchfunktion von '.config('app.name').'. Geben Sie Ihre Suchanfrage in das Feld ein. Stellen Sie sicher, dass Sie sich die Hilfe ansehen, da die Suche ziemlich umfangreich ist.',
    ...
    'export_data_intro'                          => 'Exporte können Sie verwenden um Ihre Daten in anderen Anwendungen zu importieren. Bitte beachten Sie, dass diese Dateien nicht als Backup gedacht sind. Sie enthalten nicht genügend Metadaten, um eine neue '.config('app.name').'-Installation vollständig wiederherzustellen. Wenn Sie Ihre Daten sichern möchten, sichern Sie bitte die Datenbank direkt.',
    'export_format'                              => 'Export-Format',
    'export_format_csv'                          => 'Durch Komma getrennte Werte (CSV-Datei)',
    'export_format_mt940'                        => 'MT940 kompatibles Format',
    'include_old_uploads_help'                   => config('app.name').' löscht keine originalen CSV-Dateien, welche zuvor importiert wurden. Sie können dem Export hinzugefügt werden.',
    'do_export'                                  => 'Export',
    ...
    'instructions_rule_from_bill'                => 'Um Buchungen an Ihre neue Rechnung „:name” anzupassen, kann '.config('app.name').' eine Regel erstellen, die automatisch gegen alle von Ihnen gespeicherten Buchungen geprüft wird. Bitte überprüfen Sie die folgenden Angaben und speichern Sie die Regel, damit '.config('app.name').' die Buchungen automatisch mit Ihrer neuen Rechnung abgleicht.',
    ...
    'create_rule_for_bill_txt'                => 'Sie haben gerade eine neue Rechnung namens „:name” erstellt. Herzlichen Glückwunsch! '.config('app.name').' kann automatisch neue Abbuchungen dieser Rechnung zuordnen. Zum Beispiel, wenn Sie Ihre Miete bezahlen, wird die Rechnung „Miete” an den Kostenbeitrag gebunden. Auf diese Weise kann '.config('app.name').' Ihnen genau zeigen, welche Rechnungen fällig sind und welche nicht. '.config('app.name').' hat einige sinnvolle Vorgaben für Sie ausgefüllt. Bitte stellen Sie sicher, dass diese korrekt sind. Wenn diese Werte korrekt sind, verknüpft '.config('app.name').' automatisch die korrekte Abbuchung mit der richtigen Rechnung. Bitte überprüfen Sie die Auslöser, um zu sehen, ob diese korrekt sind. Falls diese falsch sind, korrigieren Sie diese bitte.',
    ...
    'pref_languages_help'                     => config('app.name').' unterstützt mehrere Sprachen. Welche möchten Sie nutzen?',
    ...
    'secure_pw_ff'                            => 'Verwenden Sie im gesamten Internet das gleiche Passwort? Wenn eine Website Ihr Passwort verliert, haben Hacker Zugriff auf alle Ihre Daten. '.config('app.name').' verlässt sich darauf, dass Sie ein sicheres und einzigartiges Passwort wählen, um Ihre Finanzdaten zu schützen.',
    'secure_pw_check_box'                     => 'Um Ihnen dabei zu helfen, kann '.config('app.name').' überprüfen, ob das von Ihnen gewünschte Passwort in der Vergangenheit gestohlen wurde. Wenn dies der Fall ist, rät '.config('app.name').' Ihnen, dieses Passwort NICHT zu verwenden.',
    'secure_pw_working_title'                 => 'Wie funktioniert es?',
    'secure_pw_working'                       => 'Wenn Sie das Kontrollkästchen aktivieren, wird '.config('app.name').' den SHA1-Hash der ersten 5 Zeichen Ihres Passworts an <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">die Website von Troy Hunt</a> senden, um zu sehen, ob es auf der Liste steht. Dies verhindert, dass Sie unsichere Passwörter verwenden, was in der aktuellen <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">NIST-Sonderveröffentlichung</a> zu diesem Thema empfohlen wird.',
    ...
    'currencies_intro'                        => config('app.name').' unterstützt verschiedene Währungen, welche hier eingestellt und aktiviert werden können.',
    ...
    'auto_match_on'                           => 'Automatisch von '.config('app.name').' zugeordnet',
    'auto_match_off'                          => 'Nicht automatisch von '.config('app.name').' zugeordnet',
    ...
    // accounts:
    'account_missing_transaction'             => 'Konto #:id („:name”) kann nicht direkt angezeigt werden, da '.config('app.name').' Weiterleitungsinformationen fehlen.',
    ...
    'reconcile_has_more'                      => 'Ihr '.config('app.name').'-Konto verfügt über mehr Geld, als Ihre Bank behauptet. Es gibt mehrere Möglichkeiten. Bitte wählen Sie aus, was Sie tun möchten. Drücken Sie anschließend auf „Abgleich bestätigen”.',
    'reconcile_has_less'                      => 'Ihr '.config('app.name').'-Konto verfügt über weniger Geld, als Ihre Bank behauptet. Es gibt mehrere Möglichkeiten. Bitte wählen Sie aus, was Sie tun möchten. Drücken Sie dann auf „Abgleich bestätigen”.',
    'reconcile_is_equal'                      => 'Ihr '.config('app.name').'-Konto und Ihre Kontoauszüge stimmen überein. Es gibt nichts zu tun. Bitte drücken Sie auf „Abgleich bestätigen”, um Ihre Eingaben zu bestätigen.',
    ...
    // new user:
    'welcome'                                 => 'Willkommen bei '.config('app.name').'!',
    'submit'                                  => 'Absenden',
    'submit_yes_really'                       => 'Senden (Ich weiß, was ich tue)',
    'getting_started'                         => 'Erste Schritte',
    'to_get_started'                          => 'Es ist schön, dass Sie '.config('app.name').' erfolgreich installiert haben. Um mit diesem Tool zu beginnen, geben Sie bitte den Namen Ihrer Bank und das Guthaben Ihres Hauptkontos ein. Machen Sie sich keine Sorgen, wenn Sie mehrere Konten haben. Sie können diese später hinzufügen. Dies ist nur der Anfang.',
    'savings_balance_text'                    => config('app.name').' erstellt automatisch ein Sparkonto für Sie. Standardmäßig befindet sich kein Geld auf Ihrem Sparkonto, aber wenn Sie '.config('app.name').' das Guthaben mitteilen, wird es als solches gespeichert.',
    'finish_up_new_user'                      => 'Das war\'s! Sie können fortfahren, indem Sie <strong>Absenden</strong> drücken. Sie werden zur Startseite von '.config('app.name').' gebracht.',
    'stored_new_accounts_new_user'            => 'Yay! Ihre neuen Konten wurden gespeichert.',
    'set_preferred_language'                  => 'Wenn Sie '.config('app.name').' in einer anderen Sprache verwenden möchten, geben Sie dies bitte hier an.',
    'language'                                => 'Sprache',
    ...
    'firefly_instance_configuration'        => 'Konfigurationsoptionen für '.config('app.name'),
    ...
    'import_index_title'                    => 'Buchungen in '.config('app.name').' importieren',
    'import_data'                           => 'Daten importieren',
    'import_transactions'                   => 'Buchungen importieren',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Diese Funktion ist nicht verfügbar, wenn Sie '.config('app.name').' in einer Sandstorm.io-Umgebung verwenden.',
    ...
    'no_accounts_imperative_asset'          => 'Um '.config('app.name').' nutzen zu können, müssen Sie mindestens ein Bestandskonto anlegen. Lassen Sie uns das jetzt tun:',
    ...
    'no_recurring_intro_default'            => 'Sie verfügen noch über keine regelmäßigen Buchungen. Mit diesen können Sie '.config('app.name').' dazu einsetzen, automatisch Buchungen für Sie zu erstellen.',
    ...
    'warning_duplicates_repetitions'        => 'In seltenen Fällen werden die Daten zweimal in dieser Liste angezeigt. Dies kann passieren, wenn mehrere Wiederholungen aufeinandertreffen. '.config('app.name').' erzeugt immer eine Transaktion pro Tag.',
    ...
    'recurring_never_cron'                  => 'Es scheint, dass der Cron-Job, der notwendig ist, um wiederkehrende Buchungen zu unterstützen, nie ausgeführt wurde. Das ist natürlich normal, wenn Sie gerade '.config('app.name').' installiert haben, aber dies sollte so schnell wie möglich eingerichtet werden. Bitte besuchen Sie die Hilfeseiten über das ❓-Symbol in der oberen rechten Ecke der Seite.',
    ...
    'help_first_date_no_past'            => 'Geben Sie die erste erwartete Wiederholung an. '.config('app.name').' erzeugt keine Buchungen die in der Vergangenheit liegen.',
    ...
    'help_weekend'                       => 'Was sollte '.config('app.name').' tun, wenn der Dauerauftrag auf einen Samstag oder Sonntag fällt?',
