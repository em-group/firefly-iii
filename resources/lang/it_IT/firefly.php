...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Se si esegue la propria istanza di '.config('app.name').', controllare i log in <code>storage/logs</code> per istruzioni.',
    ...
    'need_more_help'                             => 'Se hai bisogno di ulteriore aiuto con '.config('app.name').', ti preghiamo di <a href="https://github.com/firefly-iii/firefly-iii/issues">aprire un ticket su Github</a>.',
    ...
    'cannot_redirect_to_account'                 => 'Spiacente ma '.config('app.name').' non può reindirizzarti alla pagina corretta.',
    ...
    'invalid_locale_settings'                    => config('app.name').' non è in grado di formattare gli importi monetari perché al server mancano i pacchetti richiesti. Ci sono <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">istruzioni su come eseguire questa operazione</a>.',
    ...
    'login_page_title'                           => 'Accedi a '.config('app.name'),
    'register_page_title'                        => 'Registrati a '.config('app.name'),
    'forgot_pw_page_title'                       => 'Hai dimenticato la password per '.config('app.name'),
    'reset_pw_page_title'                        => 'Reimposta la password per '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version Richiesta Autorizzazione',
    ...
    'admin_update_check_explain'                 => config('app.name').' può verificare automaticamente gli aggiornamenti. Quando attivi questa impostazione, contatterà Github per verificare se è disponibile una nuova versione di '.config('app.name').'. Quando lo sarà, riceverai una notifica. Puoi testare questa notifica usando il pulsante a destra. Indicare di seguito se si desidera che '.config('app.name').' controlli gli aggiornamenti.',
    'check_for_updates_permission'               => config('app.name').' può controllare gli aggiornamenti, ma è necessario il tuo permesso per farlo. Vai nell\'<a href=":link">amministrazione</a> per indicare se desideri che questa funzione sia abilitata.',
    ...
    'admin_update_check_now_explain'             => 'Se si preme il pulsante, '.config('app.name').' controllerà se la versione corrente è la più recente.',
    'check_for_updates_button'                   => 'Controlla ora!',
    'update_new_version_alert'                   => 'È disponibile una nuova versione di '.config('app.name').'. Stai eseguendo v:your_version, l\'ultima versione è v:new_version che è stata rilasciata il :date.',
    ...
    'search_found_transactions'                  => config('app.name').' ha trovato :count transazioni in :time secondi.',
    'search_for_query'                           => config('app.name').' sta cercando le transazioni contenenti tutte queste parole: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Ricerca',
    'search_box_intro'                           => 'Benvenuto nella funzione di ricerca di '.config('app.name').'. Inserisci la query di ricerca nella casella. Assicurati di controllare il file della guida perché la ricerca è abbastanza avanzata.',
    ...
    'export_data_intro'                          => 'Utilizza i dati esportati per passare a una nuova applicazione finanziaria. Si noti che questi file non sono intesi come backup. Non contengono abbastanza metadati per ripristinare completamente una nuova installazione di '.config('app.name').'. Se desideri eseguire un backup dei dati, esegui direttamente il backup del database.',
    ...
    'include_old_uploads_help'                   => config('app.name').' non getta via i file CSV originali importati in passato. Puoi includerli nell\'esportazione.',
    'do_export'                                  => 'Esporta',
    ...
    'instructions_rule_from_bill'                => 'Per abbinare le transazioni alla tua nuova bolletta ":name" '.config('app.name').' può creare una regola verrà automaticamente controllata per ogni transazione che memorizzi. Verifica i dettagli sottostanti e salva la regola perché '.config('app.name').' abbini le transazioni alla tua nuova bolletta.',
    ...
    'apply_rule_selection_intro'                 => 'Regole come ":title" sono normalmente applicate solo a transazioni nuove o aggiornate, ma puoi dire a '.config('app.name').' di eseguirle su una selezione delle tue transazioni esistenti. Questo può essere utile quando hai aggiornato una regola e hai bisogno che le modifiche vengano applicate a tutte le altre transazioni.',
    ...
    'apply_rule_group_selection_intro'           => 'Gruppi di regole come ":title" sono normalmente applicati solo a transazioni nuove o aggiornate, ma puoi dire a '.config('app.name').' di eseguire tutte le regole in questo gruppo su una selezione delle tue transazioni esistenti. Questo può essere utile quando hai aggiornato un gruppo di regole e hai bisogno delle modifiche da applicare a tutte le tue altre transazioni.',
    ...
    'create_rule_for_bill_txt'                => 'Congratulazioni, hai appena creato una nuova bolletta chiamata ":name"! '.config('app.name').' può automagicamente abbinare le nuove uscite a questa bolletta. Per esempio, ogni volta che paghi l\'affitto la bolletta "affitto" verrà collegata a questa spesa. In questo modo '.config('app.name').' può visualizzare con accuratezza quali bollette sono in scadenza e quali no. Per far ciò è necessario creare una nuova regola. '.config('app.name').' ha inserito al posto tuo alcuni dettagli ragionevoli. Assicurati che questi siano corretti. Se questi valori sono corretti, '.config('app.name').' automaticamente collegherà il prelievo giusto alla bolletta giusta. Controlla che i trigger siano corretti e aggiungine altri se sono sbagliati.',
    ...
    'pref_languages_help'                     => config('app.name').' supporta diverse lingue. Quale di queste preferisci?',
    ...
    'delete_your_account_help'                => 'L\'eliminazione del tuo account eliminerà anche conti, transazioni, <em>qualsiasi cosa</em> che potresti aver salvato in '.config('app.name').'. Sarà tutto PERDUTO.',
    ...
    'secure_pw_ff'                            => 'Utilizzi la stessa password ovunque in Internet? Se un sito perde la tua password, gli hacker hanno accesso a tutti i tuoi dati. '.config('app.name').' si affida a te nello scegliere una password robusta e unica per proteggere le tue informazioni finanziarie.',
    'secure_pw_check_box'                     => 'Per aiutarti a fare ciò, '.config('app.name').' può controllare se la password che vuoi utilizzare è stata rubata in passato. In un caso del genere, '.config('app.name').' ti consiglia di NON utilizzare quella password.',
    'secure_pw_working_title'                 => 'Come funziona?',
    'secure_pw_working'                       => 'Selezionando la casella, '.config('app.name').' invierà i primi cinque caratteri dell\'hash SHA1 della tua password al<a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">sito web di Troy Hunt</a> per vedere se è presente nell\'elenco. Questo ti impedirà di usare password non sicure come raccomandato nell\'ultima <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">pubblicazione speciale NIST</a> su questo argomento.',
    ...
    'delete_local_info_only'                  => 'Poiché ti sei autenticato tramite ":login_provider", questo eliminerà solamente le informazioni locali di '.config('app.name').'.',
    ...
    'currencies_intro'                        => config('app.name').' supporta varie valute che è possibile impostare e abilitare qui.',
    ...
    'auto_match_on'                           => 'Abbinato automaticamente da '.config('app.name'),
    'auto_match_off'                          => 'Non abbinato automaticamente a '.config('app.name'),
    ...
    'reconcile_has_more'                      => 'Il libro mastro di '.config('app.name').' ha più denaro rispetto a quanto afferma la tua banca. Ci sono diverse opzioni. Scegli cosa fare. In seguito, premi "Conferma riconciliazione".',
    'reconcile_has_less'                      => 'Il tuo conto '.config('app.name').' ha meno denaro rispetto a quanto afferma la tua banca. Ci sono diverse opzioni Si prega di scegliere cosa fare. Quindi, premi "Conferma riconciliazione".',
    'reconcile_is_equal'                      => 'Il libro mastro di '.config('app.name').' e i tuoi estratti conto bancari corrispondono. Non c\'è niente da fare. Premi "Conferma riconciliazione" per confermare l\'inserimento.',
    ...
    // new user:
    'welcome'                                 => 'Benvenuto in '.config('app.name').'!',
    'submit'                                  => 'Invia',
    'submit_yes_really'                       => 'Invia (sì, so quello che sto facendo)',
    'getting_started'                         => 'Inizia',
    'to_get_started'                          => 'È bello vedere che hai installato '.config('app.name').' con successo. Per iniziare con questo strumento, inserisci il nome della tua banca e il saldo del tuo conto corrente principale. Non preoccuparti se hai più conti. È possibile aggiungere quelli più tardi. '.config('app.name').' ha bisogno di qualcosa per iniziare.',
    'savings_balance_text'                    => config('app.name').' creerà automaticamente un conto di risparmio per te. Per impostazione predefinita, non ci saranno soldi nel tuo conto di risparmio, ma se comunichi a '.config('app.name').' il saldo verrà salvato come tale.',
    'finish_up_new_user'                      => 'Questo è tutto! Puoi continuare premendo <strong>Invia</strong>. Verrai indirizzato all\'indice di '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'I tuoi nuovi conti sono stati salvati.',
    'set_preferred_language'                  => 'Se preferisci usare '.config('app.name').' in un\'altra lingua, impostala qui.',
    'language'                                => 'Lingua',
    ...
    'import_index_title'                    => 'Importa le transazioni in '.config('app.name'),
    'import_data'                           => 'Importa dati',
    'import_transactions'                   => 'Importa transazioni',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Questa funzione non è disponibile quando si utilizza '.config('app.name').' in un ambiente Sandstorm.io.',
    ...
    'no_accounts_imperative_asset'          => 'Per iniziare a utilizzare '.config('app.name').' è necessario creare almeno un conto attività. Facciamolo ora:',
    ...
    'no_recurring_intro_default'            => 'Non hai ancora una transazione ricorrente. Puoi utilizzare queste per lasciare che '.config('app.name').' crei automaticamente le transazioni per te.',
    ...
    'warning_duplicates_repetitions'        => 'In alcuni casi rari è possibile che delle date appaiano due volte in questa lista. Questo può succedere quando ripetizioni multiple collidono. '.config('app.name').' si assicurerà sempre di generare una sola transazione per giorno.',
    ...
    'recurring_never_cron'                  => 'Sembra che il job cron necessario per le transazioni ricorrenti non sia mai stato eseguito. Questo è ovviamente normale quando hai appena installato '.config('app.name').', tuttavia dovrebbe essere impostato il prima possibile. Consulta le pagine di aiuto usando l\'icona (?) nell\'angolo in alto a destra della pagina.',
    ...
    'help_first_date_no_past'            => 'Indica quando la ricorrenza dovrebbe avvenire per la prima volta. '.config('app.name').' non creerà transazioni nel passato.',
    ...
    'help_weekend'                       => 'Cosa vuoi che '.config('app.name').' faccia quando la transazione ricorrente cade di sabato o domenica?',
