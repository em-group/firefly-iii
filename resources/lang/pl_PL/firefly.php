...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Jeśli jesteś właścicielem tej instalacji '.config('app.name').', sprawdź pliki w katalogu <code>storage/logs</code> aby uzyskać instrukcje.',
    ...
    'need_more_help'                             => 'Jeśli potrzebujesz dodatkowej pomocy w korzystaniu z '.config('app.name').', proszę <a href="https://github.com/firefly-iii/firefly-iii/issues">opisz go w zgłoszeniu na Githubie</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' nie może Cię przekierować na właściwą stronę. Przepraszamy.',
    ...
    'invalid_locale_settings'                    => config('app.name').' nie może sformatować kwot, ponieważ brakuje wymaganych pakietów na Twoim serwerze. Sprawdź<a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">instrukcje jak je zainstalować</a> (po angielsku).',
    ...
    'login_page_title'                           => 'Zaloguj się do '.config('app.name'),
    'register_page_title'                        => 'Zarejestruj się w '.config('app.name'),
    'forgot_pw_page_title'                       => 'Nie pamiętam hasła do '.config('app.name'),
    'reset_pw_page_title'                        => 'Resetowanie hasła do '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => 'Żądanie autoryzacji '.config('app.name').' v:version',
    ...
    'admin_update_check_explain'                 => config('app.name').' może automatycznie sprawdzać dostępność aktualizacji. Po włączeniu tej opcji będzie kontaktować się z Githubem w celu sprawdzenia czy dostępna jest nowa wersja '.config('app.name').'. Jeżeli będzie, otrzymasz powiadomienie. Możesz przetestować to powiadomienie używając przycisku po prawej stronie. Wskaż proszę czy chcesz, aby '.config('app.name').' sprawdzało dostępność aktualizacji.',
    'check_for_updates_permission'               => config('app.name').' może sprawdzić dostępność aktualizacji, ale potrzebuje Twojej zgody, aby to robić. Przejdź do <a href=":link">administracji</a>, aby określić, czy ta funkcja ma być włączona.',
    ...
    'admin_update_check_now_explain'             => 'Jeśli naciśniesz przycisk, '.config('app.name').' sprawdzi, czy używasz najnowszej wersji.',
    'check_for_updates_button'                   => 'Sprawdź teraz!',
    'update_new_version_alert'                   => 'Dostępna jest nowa wersja '.config('app.name').'. Używasz wersji :your_version, najnowsza wersja to :new_version wydana :date.',
    ...
    'search_found_transactions'                  => config('app.name').' znalazło :count transakcji w :time sekundy.',
    'search_for_query'                           => config('app.name').' szuka transakcji zawierających wszystkie słowa: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Szukaj',
    'search_box_intro'                           => 'Witamy w funkcji wyszukiwania '.config('app.name').'. Wpisz zapytanie w polu. Upewnij się, że sprawdziłeś plik pomocy, ponieważ wyszukiwanie jest dość zaawansowane.',
    ...
    'export_data_intro'                          => 'Użyj wyeksportowanych danych, aby przejść do nowej aplikacji finansowej. Należy pamiętać, że te pliki są kopią zapasową. Nie zawierają one wystarczającej ilości metadanych, aby w pełni przywrócić nową instalację '.config('app.name').'. Jeśli chcesz wykonać kopię zapasową danych, wykonaj bezpośrednio kopię bazy danych.',
    ...
    'include_old_uploads_help'                   => config('app.name').' nie wyrzuca oryginalnych plików CSV zaimportowanych w przeszłości. Możesz uwzględnić je w eksporcie.',
    'do_export'                                  => 'Eksportuj',
    ...
    'instructions_rule_from_bill'                => 'Aby dopasować transakcje do nowego rachunku ":name", '.config('app.name').' może utworzyć regułę, która będzie automatycznie sprawdzana względem wszelkich transakcji, które zapisujesz. Sprawdź poniższe szczegóły i zapisz regułę, aby '.config('app.name').' automatycznie dopasowywało transakcje do Twojego nowego rachunku.',
    ...
    'apply_rule_selection_intro'                 => 'Reguły takie jak ":title" są zwykle stosowane tylko do nowych lub modyfikowanych transakcji, ale możesz powiedzieć '.config('app.name').' aby uruchomił ją dla istniejących transakcji. Może to być przydatne, gdy zmodyfikowałeś regułę i potrzebujesz zastosować zmiany dla wszystkich pozostałych transakcji.',
    ...
    'apply_rule_group_selection_intro'           => 'GRupy reguł takie jak ":title" są zwykle stosowane tylko do nowych lub modyfikowanych transakcji, ale możesz powiedzieć '.config('app.name').' aby uruchomił ją dla istniejących transakcji. Może to być przydatne, gdy zmodyfikowałeś grupę reguł i potrzebujesz zastosować zmiany dla wszystkich pozostałych transakcji.',
    ...
    'create_rule_for_bill_txt'                => 'Właśnie utworzyłeś nowy rachunek o nazwie ":name". Gratulacje! '.config('app.name').' może automagicznie dopasować nowe wypłaty do tego rachunku. Na przykład za każdym razem, gdy płacisz czynsz, rachunek "czynsz" zostanie powiązany z wydatkiem. W ten sposób '.config('app.name').' może dokładnie pokazać, które rachunki są jeszcze do zapłacenia, a które nie. W tym celu należy utworzyć nową regułę. '.config('app.name').' wypełnił dla Ciebie pewne rozsądne domyślne ustawienia. Upewnij się, że są poprawne. Jeśli są, '.config('app.name').' automatycznie połączy pasującą wypłatę z prawidłowym rachunkiem. Sprawdź wyzwalacze czy są poprawne i dodaj kolejne jeśli są błędne.',
    ...
    'pref_languages_help'                     => config('app.name').' obsługuje kilka języków. Który wolisz?',
    ...
    'delete_your_account_help'                => 'Usunięcie twojego konta usunie również wszystkie konta, transakcje, <em>wszystko</em> co mogłeś mieć zapisane w '.config('app.name').'. Po prostu ZNIKNIE.',
    ...
    'secure_pw_ff'                            => 'Czy używasz tego samego hasła w całym Internecie? Jeśli jedna strona straci Twoje hasło, hakerzy będą mieli dostęp do wszystkich Twoich danych. '.config('app.name').' polega na Twoim wyborze silnego i unikatowego hasła w celu ochrony Twojej dokumentacji finansowej.',
    'secure_pw_check_box'                     => 'Aby Ci pomóc, '.config('app.name').' może sprawdzić, czy hasło, którego chcesz użyć, zostało w przeszłości skradzione. Jeśli okaże się to prawdą, '.config('app.name').' radzi NIE używać tego hasła.',
    'secure_pw_working_title'                 => 'Jak to działa?',
    'secure_pw_working'                       => 'Zaznaczając to pole, '.config('app.name').' wyśle pierwsze pięć znaków skrótu SHA1 Twojego hasła do <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">strony internetowej Troy Hunt</a>, aby sprawdzić, czy znajduje się ono na liście haseł, które wyciekły. To powstrzyma Cię przed używaniem niebezpiecznych haseł, co jest zalecane w najnowszej <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">specjalnej publikacji Narodowy Instytut Standaryzacji i Technologii (NIST)</a> na ten temat.',
    ...
    'email_verification'                      => 'Wiadomość e-mail zostanie wysłana na stary ORAZ nowy adres e-mail. Ze względów bezpieczeństwa, nie będziesz mógł się zalogować, dopóki nie zweryfikujesz nowego adresu e-mail. Jeśli nie masz pewności, czy twoja instalacja '.config('app.name').' umożliwia wysyłanie wiadomości e-mail, nie korzystaj z tej funkcji. Jeśli jesteś administratorem, możesz przetestować to w <a href="/admin">panelu administracyjnym</a>.',
    ...
    'delete_local_info_only'                  => 'Ponieważ uwierzytelniasz się poprzez ":login_provider", usunięte zostaną tylko lokalne dane '.config('app.name').'.',
    ...
    'currencies_intro'                        => config('app.name').' obsługuje różne waluty, które można ustawić i włączyć tutaj.',
    ...
    'auto_match_on'                           => 'Automatycznie dopasowane przez '.config('app.name'),
    'auto_match_off'                          => 'Niedopasowane automatycznie przez '.config('app.name'),
    ...
    // accounts:
    'account_missing_transaction'             => 'Konto #:id (":name") nie możne być wyświetlone bezpośrednio, ale '.config('app.name').' nie ma informacji przekierowania.',
    ...
    'reconcile_has_more'                      => 'Twoja księga główna '.config('app.name').' ma więcej pieniędzy niż bank twierdzi, że powinieneś mieć. Istnieje kilka opcji. Wybierz, co zrobić. Następnie naciśnij "Potwierdź uzgodnienie".',
    'reconcile_has_less'                      => 'Twoja księga główna '.config('app.name').' ma mniej pieniędzy niż bank twierdzi, że powinieneś mieć. Istnieje kilka opcji. Wybierz, co zrobić. Następnie naciśnij "Potwierdź uzgodnienie".',
    'reconcile_is_equal'                      => 'Twoja księga główna '.config('app.name').' i wyciągi bankowe są zgodne. Nie ma nic do zrobienia. Naciśnij "Potwierdź uzgodnienie" aby potwierdzić twój wybór.',
    ...
    // new user:
    'welcome'                                 => 'Witaj w '.config('app.name').'!',
    'submit'                                  => 'Prześlij',
    'submit_yes_really'                       => 'Zatwierdź (wiem co robię)',
    'getting_started'                         => 'Pierwsze kroki',
    'to_get_started'                          => 'Dobrze, że udało ci się zainstalować '.config('app.name').'. Aby rozpocząć korzystanie z tego narzędzia, wprowadź nazwę swojego banku i saldo głównego rachunku bieżącego. Nie martw się, jeśli masz wiele kont. Możesz dodać je później. Po prostu '.config('app.name').' potrzebuje czegoś na początek.',
    'savings_balance_text'                    => config('app.name').' automatycznie utworzy dla ciebie konto oszczędnościowe. Domyślnie na twoim koncie oszczędnościowym nie ma pieniędzy, ale jeśli chcesz, '.config('app.name').' może je tam przechowywać.',
    'finish_up_new_user'                      => 'To wszystko! Możesz kontynuować, naciskając <strong>Zatwierdź</strong>. Zostaniesz przeniesiony do strony głównej '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'Yay! Twoje nowe konta zostały zapisane.',
    'set_preferred_language'                  => 'Jeśli wolisz używać '.config('app.name').' w innym języku, wskaż to tutaj.',
    'language'                                => 'Język',
    ...
    'firefly_instance_configuration'        => 'Opcje konfiguracji dla '.config('app.name'),
    'setting_single_user_mode'              => 'Tryb pojedynczego użytkownika',
    'setting_single_user_mode_explain'      => 'Domyślnie, '.config('app.name').' pozwala na jednego (1) użytkownika: Ciebie. Jest to środek bezpieczeństwa uniemożliwiający innym używanie Twojej instalacji, chyba że im pozwolisz. Kolejne rejestracje są zablokowane. Jeżeli odznaczysz to pole, inne osoby będą mogły używać Twojej instalacji '.config('app.name').' (zakładając, że jest ona dostępna w Internecie).',
    ...
    'import_index_title'                    => 'Importuj transakcje do '.config('app.name'),
    'import_data'                           => 'Importuj dane',
    'import_transactions'                   => 'Importuj transakcje',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Ta funkcja nie jest dostępna, gdy używasz '.config('app.name').' w środowisku Sandstorm.io.',
    ...
    'no_accounts_imperative_asset'          => 'Aby rozpocząć korzystanie z '.config('app.name').' należy utworzyć co najmniej jedno konto aktywów. Zróbmy je więc teraz:',
    ...
    'no_recurring_intro_default'            => 'Nie masz jeszcze żadnych cyklicznych transakcji. Możesz ich użyć, aby '.config('app.name').' automatycznie tworzył transakcje za Ciebie.',
    ...
    'warning_duplicates_repetitions'        => 'W rzadkich przypadkach daty pojawiają się dwa razy na tej liście. Może się to zdarzyć, gdy zdarzy się wiele powtórzeń. '.config('app.name').' zawsze generuje jedną transakcję dziennie.',
    ...
    'help_first_date_no_past'            => 'Wskaż pierwsze oczekiwane wystąpienie. '.config('app.name').' nie będzie tworzyć transakcji w przeszłości.',
    ...
    'help_weekend'                       => 'Co powinno zrobić '.config('app.name').', jeśli cykliczna transakcja wypadnie w sobotę lub niedzielę?',
