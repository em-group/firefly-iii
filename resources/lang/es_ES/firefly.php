...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Si usted ejecuta su propia instancia de '.config('app.name').', revise los registros en <code>storage/logs</code> para tener instrucciones.',
    ...
    'need_more_help'                             => 'Si necesita más ayuda con '.config('app.name').', por favor <a href="https://github.com/firefly-iii/firefly-iii/issues">abre un ticket en Github</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' no puede redirigirlo a la página correcta. Disculpas.',
    ...
    'invalid_locale_settings'                    => config('app.name').' no puede formatear cantidades monetarias porque a su servidor le faltan los paquetes requeridos. Hay <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">instrucciones de cómo hacer esto</a>.',
    ...
    'login_page_title'                           => 'Iniciar sesión en '.config('app.name'),
    'register_page_title'                        => 'Registrarse en '.config('app.name'),
    'forgot_pw_page_title'                       => 'Olvidó su contraseña para '.config('app.name'),
    'reset_pw_page_title'                        => 'Restablecer contraseña para '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version Solicitud de autorización',
    ...
    'admin_update_check_explain'                 => config('app.name').' puede chequear actualizaciones automáticamente. cuando usted habilita esta configuración, contactara Github para ver si hay disponible una nueva versión de '.config('app.name').'. Cuando sea así, usted tendrá una notificación. Usted puede probar esta notificación usando el boton de la derecha. Por favor indique abajo si quiere que '.config('app.name').' chequee actualizaciones.',
    'check_for_updates_permission'               => config('app.name').' puede chequear actualizaciones, pero necesita su permiso para hacerlo. Por favor diríjase a <a href=":link">administracion</a>para indicar si desea que esta presentación sea habilitada.',
    ...
    'admin_update_check_now_explain'             => 'SI usted presiona el botón, '.config('app.name').' vera si su actual versión es la última.',
    'check_for_updates_button'                   => '¡Ver ahora!',
    'update_new_version_alert'                   => 'Una nueva versión de '.config('app.name').' esta disponible. Usted esta ejecutando v:your_version, la ultima versión es v:new_version que se lanzo en :date.',
    ...
    'search_found_transactions'                  => config('app.name').' encontró :count transacción(es) en :time segundos.',
    'search_for_query'                           => config('app.name').' está buscando transacciones que contengan todas estas palabras: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Buscar',
    'search_box_intro'                           => 'Bienvenido a la función de búsqueda de '.config('app.name').'. Introduce tu consulta de búsqueda en el cuadro. Asegúrate de revisar el archivo de ayuda porque el buscador es bastante avanzado.',
    ...
    'instructions_rule_from_bill'                => 'A fin de hacer coincidir las transacciones con su nueva cuenta ":name", '.config('app.name').' puede crear una regla que chequeará automáticamente todas las transacciones que guardes. Por favor, verifique los detalles debajo y guarde la regla para que '.config('app.name').' automáticamente haga coincidir las transacciones con su nueva cuenta.',
    ...
    'apply_rule_selection_intro'                 => 'Las reglas como ":title" normalmente sólo se aplican a transacciones nuevas o actualizadas, pero puedes indicarle a '.config('app.name').' que las ejecute en una selección de transacciones existentes. Esto puede ser útil si has actualizado una regla y necesitas que los cambios se apliquen a todas tus otras transacciones.',
    ...
    'apply_rule_group_selection_intro'           => 'Los grupos de reglas como ":title" normalmente sólo se aplican a transacciones nuevas o actualizadas, pero puedes indicarle a '.config('app.name').' que ejecute todas las reglas de este grupo en una selección de transacciones existentes. Esto puede ser útil si has actualizado un grupo de reglas y necesitas que los cambios se apliquen a todas tus otras transacciones.',
    ...
    'create_rule_for_bill_txt'                => 'Acaba de crear una nueva factura llamada ":name", ¡enhorabuena! '.config('app.name').' puede asociar de manera automágica nuevas retiradas con esta factura. Por ejemplo, siempre que pague su alquiler, la factura "alquiler" será enlazada a ese gasto. De esta manera, '.config('app.name').' puede mostrarle de forma precisa qué facturas han vencido y cuáles no. Para ello, una nueva regla debe ser creada. Si estos valores son correctos, '.config('app.name').' enlazará automáticamente el retiro correspondiente con la factura correcta. Por favor, revise los desencadenadores para ver si son correctos, y añada algunos si son incorrectos.',
    ...
    'pref_languages_help'                     => config('app.name').' apoya varios idiomas. cual usted prefiere?',
    ...
    'delete_your_account_help'                => 'Eliminando su cuenta también eliminara cualquier otra cuenta, transacciones,<em>cualquier</em>que podría haber salvado en '.config('app.name').'. se habrá IDO.',
    ...
    'secure_pw_ff'                            => '¿Utiliza la misma contraseña a través de Internet? Si un sitio pierde su contraseña, los hackers tienen acceso a todos sus datos. '.config('app.name').' depende de usted para elegir una contraseña fuerte y única para proteger sus registros financieros.',
    'secure_pw_check_box'                     => 'Para ayudarte a hacer eso '.config('app.name').' puede comprobar si la contraseña que quieres usar ha sido robada en el pasado. Si este es el caso, '.config('app.name').' te aconseja NO usar esa contraseña.',
    'secure_pw_working_title'                 => '¿Cómo funciona?',
    'secure_pw_working'                       => 'Marcando la casilla, '.config('app.name').' enviará los cinco primeros caracteres del hash SHA1 de su contraseña al <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">sitio web de Troy Hunt</a> para ver si esta en la lista. Esto le impedirá a usted usar contraseñas inseguras como se recomienda en la última <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">Publicación Especial de NISH</a> sobre este tema.',
    ...
    'delete_local_info_only'                  => 'Debido a que se identificado a través de ":login_provider", sólo se eliminará la información local de '.config('app.name').'.',
    ...
    'currencies_intro'                        => config('app.name').' respalda varias monedas que usted puede establecer y habilitar aquí.',
    ...
    'auto_match_on'                           => 'Automáticamente igualado por '.config('app.name'),
    'auto_match_off'                          => 'No igualado automaticamente por '.config('app.name'),
    ...
    'reconcile_has_more'                      => 'Su libro principal de '.config('app.name').' tiene mas dinero de lo que su banco afirma debería tener, Hay varias opciones. por favor elija que hacer. luego, presione "confirmar reconciliación".',
    'reconcile_has_less'                      => 'Su libro principal de '.config('app.name').' tiene menos dinero de el que su banco dice que usted debería tener. hay varias opciones. Por favor elija que hacer, luego presione " Confirmar reconciliación".',
    'reconcile_is_equal'                      => 'Su libro principal de '.config('app.name').' y sus estados de cuenta coinciden. No hay nada que hacer. por favor presione "Confirmar reconciliación" para confirmar su entrada.',
    ...
    // new user:
    'welcome'                                 => 'Bienvenido a '.config('app.name').'!',
    'submit'                                  => 'Enviar',
    ...
    'firefly_instance_configuration'        => 'Opciones de configuración de '.config('app.name'),
    'setting_single_user_mode'              => 'Modo de usuario único',
    'setting_single_user_mode_explain'      => 'Por defecto, '.config('app.name').' solo acepta un (1) registro: Usted. Esta es una medida de seguridad que impide que otros utilicen su instancia a menos que usted lo permita. Los registros futuros están bloqueados. Cuando usted desbloquee esta casilla, otros pueden usar su instancia también, suponiendo que puedan alcanzarla ( cuando este conectada a Internet).',
    ...
    'import_index_title'                    => 'Importar transacciones a '.config('app.name'),
    'import_data'                           => 'Importar datos',
    'import_transactions'                   => 'Importar transacciones',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Esta función no esta disponible cuando usted esta utilizando '.config('app.name').' dentro de un ambiente Sandstorm.io.',
    ...
    'no_accounts_imperative_asset'          => 'Para comenzar a utilizar '.config('app.name').' Usted debe crear al menos una cuenta de activo. Vamos a hacerlo ahora:',
    ...
    'no_recurring_intro_default'            => 'Usted no tiene transacciones recurrentes aún. Puede usar esto para hacer que '.config('app.name').' cree transacciones por usted.',
    ...
    'warning_duplicates_repetitions'        => 'En algunas ocasiones, las fechas aparecen duplicadas en esta lista. Esto puede ocurrir porque múltiples repeticiones chocan. '.config('app.name').' siempre generará una transacción por día.',
    ...
    'recurring_never_cron'                  => 'Al parecer, el cron job necesario para realizar las transacciones recurrentes nunca se ejecutó. Esto es normal por supuesto, cuando acabas de instalar '.config('app.name').' pero, es algo que deberías configurar lo antes posible. Por favor, revisa las páginas de ayuda usando el ícono-(?) en la esquina derecha de la página.',
    ...
    'help_first_date_no_past'            => 'Indique la primera recurrencia esperada. '.config('app.name').' no creará transacciones en el pasado.',
    ...
    'help_weekend'                       => '¿Qué debe hacer '.config('app.name').' cuando la transacción recurrente cae el sábado o el domingo?',
