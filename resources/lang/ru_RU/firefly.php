...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Если вы запустили свой собственный сервер '.config('app.name').', ознакомьтесь с лог файлами в <code>storage/logs</code> для получения инструкций.',
    ...
    'need_more_help'                             => 'Если вам нужна дополнительная помощь по использованию '.config('app.name').', пожалуйста, <a href="https://github.com/firefly-iii/firefly-iii/issues">откройте issue на Github</a> (желательно, на английском языке).',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' не может перенаправить вас на правильную страницу. Приносим извинения.',
    ...
    'invalid_locale_settings'                    => config('app.name').' не может обеспечить правильный формат денежных сумм, поскольку на вашем сервере отсутствуют необходимые пакеты. Воспользуйтесь <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">инструкцией</a>, объясняющей, как их установить.',
    ...
    'login_page_title'                           => 'Вход в '.config('app.name'),
    'register_page_title'                        => 'Регистрация в '.config('app.name'),
    'forgot_pw_page_title'                       => 'Забыли свой пароль к '.config('app.name').'?',
    'reset_pw_page_title'                        => 'Сбросить свой пароль к '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => 'Запрос авторизации '.config('app.name').' v:version',
    ...
    'admin_update_check_explain'                 => config('app.name').' может автоматически проверять наличие обновлений. После включения опции, он свяжется с Github, чтобы узнать, доступна ли новая версия '.config('app.name').'. Когда это произойдет, вы получите уведомление. Вы можете проверить это уведомление, нажав кнопку справа. Пожалуйста, укажите ниже, если вы хотите, чтобы '.config('app.name').' проверял наличие обновлений.',
    'check_for_updates_permission'               => config('app.name').' может проверять наличие обновлений, но для этого требуется ваше разрешение. Перейдите в <a href=":link">администрирование</a>, чтобы указать, хотите ли вы включить эту функцию.',
    ...
    'admin_update_check_now_explain'             => 'Если вы нажмете кнопку, '.config('app.name').' посмотрит, является ли ваша текущая версия последней.',
    'check_for_updates_button'                   => 'Проверить сейчас!',
    'update_new_version_alert'                   => 'Доступна новая версия '.config('app.name').'. Вы используете версию v:your_version, самая свежая версия - v:new_version (выпущена :date).',
    ...
    'search'                                     => 'Поиск',
    'search_query'                               => 'Запрос',
    'search_found_transactions'                  => config('app.name').' нашёл :count транзакций за :time секунд.',
    'search_for_query'                           => config('app.name').' ищет транзакции со всеми этими словами: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Поиск',
    'search_box_intro'                           => 'Добро пожаловать в функцию поиска '.config('app.name').'. Введите поисковый запрос в поле. Убедитесь, что вы ознакомились с инструкцией, потому что поиск достаточно продвинутый.',
    ...
    'export_data_intro'                          => 'Используйте экспортированные данные для перехода на новое финансовое приложение. Обратите внимание, что эти файлы не предназначены для резервного копирования. Они не содержат достаточного количества данных, чтобы полностью восстановить новую копию '.config('app.name').'. Если вы хотите сделать резервную копию своих данных, сделайте резервную копию базы данных напрямую.',
    ...
    'include_old_uploads_help'                   => config('app.name').' не выбрасывает исходные CSV-файлы, которые вы импортировали в прошлом. Вы можете включить их в свой экспорт.',
    'do_export'                                  => 'Экспорт',
    ...
    'create_rule_for_bill_txt'                => 'Вы только что создали новый счёт на оплату под названием ":name", поздравляем! '.config('app.name').' может автоматически сопоставлять новые расходы с этим счётом. Например, всякий раз, когда вы платите арендную плату, счёт «аренда» будет связан с соответствующим расходом. Таким образом, '.config('app.name').' может точно показать вам, какие счета уже оплачены, а какие ещё нет. Для этого необходимо создать новое правило. '.config('app.name').' заполнил некоторые поля разумными значениями по умолчанию. Пожалуйста, убедитесь, что они верны. Если эти значения верны, '.config('app.name').' автоматически свяжет правильный расход с правильным счётом на оплату. Проверьте правила, чтобы убедиться, что они верны, и добавьте что-либо, если в правилах есть ошибка.',
    ...
    'pref_languages_help'                     => config('app.name').' поддерживает несколько языков. Какой язык вы предпочитаете?',
    ...
    'delete_your_account_help'                => 'При удалении вашего профиля также будут удалены все счета, транзакции. Не будет сохранено <em>ничего</em>, что вы хранили в '.config('app.name').'. Всё будет УТЕРЯНО!',
    ...
    'currencies_intro'                        => config('app.name').' может работать с несколькими валютами. Вы можете управлять ими здесь.',
    ...
    'auto_match_on'                           => 'Автоматически сверено '.config('app.name'),
    'auto_match_off'                          => 'Не сверено автоматически '.config('app.name'),
    ...
    'reconcile_has_more'                      => 'В вашей бухгалтерской книге '.config('app.name').' учтено больше денег, чем должно быть, согласно выписке из банка. Существует несколько вариантов. Пожалуйста, выберите, что делать, а затем нажмите «Подтвердить сверку».',
    'reconcile_has_less'                      => 'В вашей бухгалтерской книге '.config('app.name').' учтено меньше денег, чем должно быть, согласно выписке из банка. Существует несколько вариантов. Пожалуйста, выберите, что делать, а затем нажмите «Подтвердить сверку».',
    'reconcile_is_equal'                      => 'Ваша бухгалтерская книга '.config('app.name').' и ваши банковские выписки совпадают. Ничего делать не нужно. Пожалуйста, нажмите «Подтвердить сверку» для ввода данных.',
    ...
    'to_get_started'                          => 'Приятно видеть, что вы успешно установили '.config('app.name').'. Чтобы начать работу, введите, пожалуйста, название своего банка и баланс вашего основного банковского счёта. Если вы планируете использовать несколько счетов, не волнуйтесь, вы сможете добавить их позже. Сейчас '.config('app.name').' просто нужны какие-нибудь первоначальные данные.',
    'savings_balance_text'                    => config('app.name').' автоматически создаст сберегательный счёт для вас. По умолчанию на вашем сберегательном счёте не будет денег, но если вы укажете начальный баланс, он будет сохранен.',
    'finish_up_new_user'                      => 'Это всё! Вы можете продолжить, нажав <strong>Подтвердить</strong>. Вы попадете на главную страницу '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'Ура! Ваши новые учётные записи сохранены.',
    'set_preferred_language'                  => 'Если вы предпочитаете использовать '.config('app.name').' на другом языке, укажите здесь здесь.',
    'language'                                => 'Язык',
    ...
    'firefly_instance_configuration'        => 'Базовая конфигурация '.config('app.name'),
    'setting_single_user_mode'              => 'Режим одного пользователя',
    'setting_single_user_mode_explain'      => 'По умолчанию '.config('app.name').' работает только с одним пользователем (это вы). Это сделано с целью обеспечения безопасности, чтобы другие люди не могли использовать ваш '.config('app.name').' без вашего разрешения. Регистрация других пользователей просто невозможна. Однако, если вы снимите этот флажок, другие смогут использовать ваш Firefly, при условии, что у них есть доступ к нему (например, он доступен через Интернет).',
    ...
    'setting_is_demo_site_explain'          => 'Если вы установите этот флажок, эта копия '.config('app.name').' будет вести себя как демонстрационный сайт, который может иметь странные побочные эффекты.',
    ...
    'import_index_title'                    => 'Импорт транзакций в '.config('app.name'),
    'import_data'                           => 'Импорт данных',
    'import_transactions'                   => 'Импорт транзакций',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Эта функция недоступна, если вы используете '.config('app.name').' в среде Sandstorm.io.',
    ...
    'no_accounts_imperative_asset'          => 'Чтобы начать использовать '.config('app.name').', вы должны создать хотя бы один основной счёт. Давайте сделаем это сейчас:',
    ...
    'help_first_date_no_past'            => 'Укажите первое ожидаемое повторение. '.config('app.name').' не будет создавать транзакции ранее этой даты.',
    ...
    'help_weekend'                       => 'Что должен сделать '.config('app.name').', когда повторяющаяся транзакция попадает на субботу или воскресенье?',
