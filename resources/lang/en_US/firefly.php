...
return [
    ...
    'two_factor_lost_fix_self'                   => 'If you run your own instance of '.config('app.name').', check the logs in <code>storage/logs</code> for instructions.',
    ...
    'need_more_help'                             => 'If you need more help using '.config('app.name').', please <a href="https://github.com/firefly-iii/firefly-iii/issues">open a ticket on Github</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' cannot redirect you to the correct page. Apologies.',
    ...
    'invalid_locale_settings'                    => config('app.name').' is unable to format monetary amounts because your server is missing the required packages. There are <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">instructions how to do this</a>.',
    ...
    'login_page_title'                           => 'Login to '.config('app.name'),
    'register_page_title'                        => 'Register at '.config('app.name'),
    'forgot_pw_page_title'                       => 'Forgot your password for '.config('app.name'),
    'reset_pw_page_title'                        => 'Reset your password for '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version Authorization Request',
    ...
    'admin_update_check_explain'                 => config('app.name').' can check for updates automatically. When you enable this setting, it will contact Github to see if a new version of '.config('app.name').' is available. When it is, you will get a notification. You can test this notification using the button on the right. Please indicate below if you want '.config('app.name').' to check for updates.',
    'check_for_updates_permission'               => config('app.name').' can check for updates, but it needs your permission to do so. Please go to the <a href=":link">administration</a> to indicate if you would like this feature to be enabled.',
    ...
    'admin_update_check_now_explain'             => 'If you press the button, '.config('app.name').' will see if your current version is the latest.',
    'check_for_updates_button'                   => 'Check now!',
    'update_new_version_alert'                   => 'A new version of '.config('app.name').' is available. You are running v:your_version, the latest version is v:new_version which was released on :date.',
    ...
    'search'                                     => 'Search',
    'search_query'                               => 'Query',
    'search_found_transactions'                  => config('app.name').' found :count transaction(s) in :time seconds.',
    'search_for_query'                           => config('app.name').' is searching for transactions with all of these words in them: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Search',
    'search_box_intro'                           => 'Welcome to the search function of '.config('app.name').'. Enter your search query in the box. Make sure you check out the help file because the search is pretty advanced.',
    ...
    'export_data_intro'                          => 'Use the exported data to move to a new financial application. Please note that these files are not meant as a backup. They do not contain enough meta-data to fully restore a new '.config('app.name').' installation. If you want to make a backup of your data, please backup the database directly.',
    'export_format'                              => 'Export format',
    'export_format_csv'                          => 'Comma separated values (CSV file)',
    'export_format_mt940'                        => 'MT940 compatible format',
    'include_old_uploads_help'                   => config('app.name').' does not throw away the original CSV files you have imported in the past. You can include them in your export.',
    'do_export'                                  => 'Export',
    ...
    'instructions_rule_from_bill'                => 'In order to match transactions to your new bill ":name", '.config('app.name').' can create a rule that will automatically be checked against any transactions you store. Please verify the details below and store the rule to have '.config('app.name').' automatically match transactions to your new bill.',
    ...
    'apply_rule_selection_intro'                 => 'Rules like ":title" are normally only applied to new or updated transactions, but you can tell '.config('app.name').' to run it on a selection of your existing transactions. This can be useful when you have updated a rule and you need the changes to be applied to all of your other transactions.',
    ...
    'apply_rule_group_selection_intro'           => 'Rule groups like ":title" are normally only applied to new or updated transactions, but you can tell '.config('app.name').' to run all the rules in this group on a selection of your existing transactions. This can be useful when you have updated a group of rules and you need the changes to be applied to all of your other transactions.',
    ...
    'create_rule_for_bill_txt'                => 'You have just created a new bill called ":name", congratulations! '.config('app.name').' can automagically match new withdrawals to this bill. For example, whenever you pay your rent, the bill "rent" will be linked to the expense. This way, '.config('app.name').' can accurately show you which bills are due and which ones aren\'t. In order to do so, a new rule must be created. '.config('app.name').' has filled in some sensible defaults for you. Please make sure these are correct. If these values are correct, '.config('app.name').' will automatically link the correct withdrawal to the correct bill. Please check out the triggers to see if they are correct, and add some if they\'re wrong.',
    ...
    'pref_languages_help'                     => config('app.name').' supports several languages. Which one do you prefer?',
    ...
    'delete_your_account_help'                => 'Deleting your account will also delete any accounts, transactions, <em>anything</em> you might have saved into '.config('app.name').'. It\'ll be GONE.',
    ...
    'secure_pw_ff'                            => 'Do you use the same password all over the internet? If one site loses your password, hackers have access to all your data. '.config('app.name').' relies on you to choose a strong and unique password to protect your financial records.',
    'secure_pw_check_box'                     => 'To help you do that '.config('app.name').' can check if the password you want to use has been stolen in the past. If this is the case, '.config('app.name').' advises you NOT to use that password.',
    'secure_pw_working_title'                 => 'How does it work?',
    'secure_pw_working'                       => 'By checking the box, '.config('app.name').' will send the first five characters of the SHA1 hash of your password to <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">the website of Troy Hunt</a> to see if it is on the list. This will stop you from using unsafe passwords as is recommended in the latest <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">NIST Special Publication</a> on this subject.',
    ...
    'email_verification'                      => 'An email message will be sent to your old AND new email address. For security purposes, you will not be able to login until you verify your new email address. If you are unsure if your '.config('app.name').' installation is capable of sending email, please do not use this feature. If you are an administrator, you can test this in the <a href="/admin">Administration</a>.',
    ...
    'delete_local_info_only'                  => 'Because you authenticate through ":login_provider", this will only delete local '.config('app.name').' information.',
    ...
    'currencies_intro'                        => config('app.name').' supports various currencies which you can set and enable here.',
    ...
    'auto_match_on'                           => 'Automatically matched by '.config('app.name'),
    'auto_match_off'                          => 'Not automatically matched by '.config('app.name'),
    ...
    'reconcile_has_more'                      => 'Your '.config('app.name').' ledger has more money in it than your bank claims you should have. There are several options. Please choose what to do. Then, press "Confirm reconciliation".',
    'reconcile_has_less'                      => 'Your '.config('app.name').' ledger has less money in it than your bank claims you should have. There are several options. Please choose what to do. Then, press "Confirm reconciliation".',
    'reconcile_is_equal'                      => 'Your '.config('app.name').' ledger and your bank statements match. There is nothing to do. Please press "Confirm reconciliation" to confirm your input.',
    ...
    'to_get_started'                          => 'It is good to see you have successfully installed '.config('app.name').'. To get started with this tool please enter your bank\'s name and the balance of your main checking account. Do not worry yet if you have multiple accounts. You can add those later. It\'s just that '.config('app.name').' needs something to start with.',
    'savings_balance_text'                    => config('app.name').' will automatically create a savings account for you. By default, there will be no money in your savings account, but if you tell '.config('app.name').' the balance it will be stored as such.',
    'finish_up_new_user'                      => 'That\'s it! You can continue by pressing <strong>Submit</strong>. You will be taken to the index of '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'Yay! Your new accounts have been stored.',
    'set_preferred_language'                  => 'If you prefer to use '.config('app.name').' in another language, please indicate so here.',
    'language'                                => 'Language',
    ...
    'firefly_instance_configuration'        => 'Configuration options for '.config('app.name'),
    'setting_single_user_mode'              => 'Single user mode',
    'setting_single_user_mode_explain'      => 'By default, '.config('app.name').' only accepts one (1) registration: you. This is a security measure, preventing others from using your instance unless you allow them to. Future registrations are blocked. When you uncheck this box, others can use your instance as well, assuming they can reach it (when it is connected to the internet).',
    ...
    'whitelabel_administration'             => 'Whitelabel administration',
    ...
    'import_index_title'                    => 'Import transactions into '.config('app.name'),
    'import_data'                           => 'Import data',
    'import_transactions'                   => 'Import transactions',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'This function is not available when you are using '.config('app.name').' within a Sandstorm.io environment.',
    ...
    'no_accounts_imperative_asset'          => 'To start using '.config('app.name').' you must create at least one asset account. Let\'s do so now:',
    ...
    'no_recurring_intro_default'            => 'You have no recurring transactions yet. You can use these to make '.config('app.name').' automatically create transactions for you.',
    ...
    'warning_duplicates_repetitions'        => 'In rare instances, dates appear twice in this list. This can happen when multiple repetitions collide. '.config('app.name').' will always generate one transaction per day.',
    ...
    'recurring_never_cron'                  => 'It seems the cron job that is necessary to support recurring transactions has never run. This is of course normal when you have just installed '.config('app.name').', but this should be something to set up as soon as possible. Please check out the help-pages using the (?)-icon in the top right corner of the page.',
    ...
    'help_first_date_no_past'            => 'Indicate the first expected recurrence. '.config('app.name').' will not create transactions in the past.',
    ...
    'help_weekend'                       => 'What should '.config('app.name').' do when the recurring transaction falls on a Saturday or Sunday?',
