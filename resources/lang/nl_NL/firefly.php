...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Als dit jouw installatie van '.config('app.name').' is, vind je in de logboeken (<code>storage/logs</code>) instructies.',
    ...
    'need_more_help'                             => 'Als je meer hulp nodig hebt met '.config('app.name').', <a href="https://github.com/firefly-iii/firefly-iii/issues">open dan een ticket op Github</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' kan je niet naar de goede pagina doorsturen. Sorry!',
    ...
    'invalid_locale_settings'                    => config('app.name').' kan geldbedragen niet goed weergeven omdat je server de vereiste software mist. Er zijn <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">instructies hoe dit te doen</a>.',
    ...
    'login_page_title'                           => 'Inloggen bij '.config('app.name'),
    'register_page_title'                        => 'Registreren bij '.config('app.name'),
    'forgot_pw_page_title'                       => 'Je wachtwoord voor '.config('app.name').' vergeten',
    'reset_pw_page_title'                        => 'Reset je '.config('app.name').' wachtwoord',
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version autorisatieverzoek',
    ...
    'admin_update_check_explain'                 => config('app.name').' kan automatisch op updates controleren. Wanneer je deze instelling inschakelt, neemt '.config('app.name').' contact op met Github om te kijken of er een nieuwe versie van '.config('app.name').' beschikbaar is. Wanneer dit het geval is, ontvang je een melding. Je kan deze melding testen met de knop aan de rechterkant. Geef hieronder aan of je '.config('app.name').' wilt laten controleren op updates.',
    'check_for_updates_permission'               => config('app.name').' kan controleren op updates, maar heeft hiervoor je toestemming nodig. Ga naar <a href=":link">de admin</a> om aan te geven of deze functie moet worden ingeschakeld.',
    ...
    'admin_update_check_now_explain'             => 'Als u op de knop drukt, kijkt '.config('app.name').' of je huidige versie de nieuwste is.',
    'check_for_updates_button'                   => 'Controleer nu!',
    'update_new_version_alert'                   => 'Er is een nieuwe versie van '.config('app.name').' beschikbaar. Je gebruikt v:your_version, de nieuwste versie is v:new_version die werd uitgebracht op :date.',
    ...
    'search_found_transactions'                  => config('app.name').' vond :count transactie(s) in :time seconden.',
    'search_for_query'                           => config('app.name').' zoekt transacties met al deze woorden: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Zoeken',
    'search_box_intro'                           => 'Welkom bij de zoekfunctie van '.config('app.name').'. Voer je zoekopdracht in het vak. Bekijk zeker de help-pagina, want de zoekfunctie is behoorlijk geadvanceerd.',
    ...
    'export_data_intro'                          => 'Gebruik de geëxporteerde data om naar een nieuwe financiële applicatie te gaan. Gebruik deze bestanden niet als backup. Er zit niet genoeg metadata bij om een nieuwe '.config('app.name').' mee op te zetten. Als je een backup wilt maken van je gegevens, backup dan de database zelf.',
    'export_format'                              => 'Exporteerformaat',
    'export_format_csv'                          => 'Komma-gescheiden bestand (CSV)',
    'export_format_mt940'                        => 'MT940 bestand',
    'include_old_uploads_help'                   => config('app.name').' gooit je oude geïmporteerde CSV bestanden niet weg. Je kan ze meenemen in je exportbestand.',
    ...
    'instructions_rule_from_bill'                => 'Om transacties te kunnen koppelen aan je nieuwe contract ":name" moet '.config('app.name').' een nieuwe regel maken die automatisch checkt of nieuwe transacties horen bij dit nieuwe contract. Check de details hieronder en sla de nieuwe regel op, zodat '.config('app.name').' dit klusje voor je kan klaren.',
    ...
    'apply_rule_selection_intro'                 => 'Regels zoals ":title" worden normaal alleen op nieuwe of geüpdate transacties toegepast, maar '.config('app.name').' kan ze ook toepassen op (een selectie van) je bestaande transacties. Dit kan praktisch zijn als je een regels hebt veranderd en je wilt de veranderingen toepassen op al je transacties.',
    ...
    'create_rule_for_bill_txt'                => 'Je hebt een nieuw contract ":name" gemaakt, hoera! '.config('app.name').' can automagisch naar je uitgaven kijken en deze koppelen aan dit contract. Als je bijvoorbeeld je huur betaalt wordt de uitgave gekoppeld aan contract "huur". Op die manier laat '.config('app.name').' je zien welke rekeningen nog betaald moeten worden en welke niet. Om dat te doen moet er een regel gemaakt worden. '.config('app.name').' heeft vast wat handige waardes ingevuld. Dubbelcheck deze. Als ze kloppen zal '.config('app.name').' automatisch de juiste uitgave aan het juiste contract koppelen. Kijk dus goed of de triggers kloppen en corrigeer waar nodig.',
    ...
    'pref_languages_help'                     => config('app.name').' ondersteunt meerdere talen. Welke heeft jouw voorkeur?',
    ...
    'pref_two_factor_auth_remove_it'          => 'Vergeet niet om je '.config('app.name').' account uit je authenticatie appje te verwijderen!',
    ...
    'secure_pw_ff'                            => 'Gebruik je overal hetzelfde wachtwoord? Als één site je wachtwoord kwijt raakt hebben hackers toegang tot al je data. '.config('app.name').' rekent er op dat je een sterk en uniek wachtwoord kiest om je financiële gegevens te beschermen.',
    'secure_pw_check_box'                     => 'Om je daarbij te helpen kan '.config('app.name').' checken of het wachtwoord dat je wilt gebruiken is gestolen in het verleden. Als dat zo is, raadt '.config('app.name').' je aan om dat wachtwoord NIET te gebruiken.',
    'secure_pw_working_title'                 => 'Hoe werkt het?',
    'secure_pw_working'                       => 'Als je het vinkje aanvinkt stuurt '.config('app.name').' de eerste vijf karakters van de SHA1-hash van je wachtwoord naar <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">de website van Troy Hunt</a> om te zien of-ie op de lijst staat. Dit voorkomt dat je een onveilig wachtwoord gebruikt, zoals is voorgeschreven in <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">een speciale publicatie van het NIST</a> over dit onderwerp.',
    ...
    'email_verification'                      => 'Het systeem verstuurt een mailtje naar je oude EN nieuwe emailadres. Uit veiligheidsoverwegingen ben je zometeen uitgelogd en kan je pas weer inloggen als je je emailadres hebt bevestigd. Doe dit niet als je niet zeker weet of je '.config('app.name').' installatie wel mail kan versturen. Als je een admin bent, test dit dan eerst in de <a href="/admin">administratie</a>.',
    ...
    'delete_local_info_only'                  => 'Omdat je inlogt via ":login_provider" verwijder je alleen lokale '.config('app.name').' informatie.',
    ...
    'currencies_intro'                        => config('app.name').' ondersteunt diverse valuta die je hier kan instellen en bewerken.',
    ...
    'auto_match_on'                           => 'Automatisch herkend door '.config('app.name'),
    'auto_match_off'                          => 'Niet automatisch herkend door '.config('app.name'),
    ...
    // accounts:
    'account_missing_transaction'             => 'Rekening #:id (":name") kan je niet bekijken in '.config('app.name').', maar het origineel is kwijt.',
    ...
    'reconcile_has_more'                      => 'Je '.config('app.name').' boekhouding bevat meer geld dan je bank beweert dat er in zou moeten zitten. Je hebt verschillende opties. Kies wat je wilt doen, en klik dan "Bevestig afstemming".',
    'reconcile_has_less'                      => 'Je '.config('app.name').' boekhouding bevat minder geld dan je bank beweert dat er in zou moeten zitten. Je hebt verschillende opties. Kies wat je wilt doen, en klik dan "Bevestig afstemming".',
    'reconcile_is_equal'                      => 'Je '.config('app.name').' boekhouding en je bankafschriften lopen gelijk. Je hoeft niets te doen. Klik op "Bevestig afstemming" om je invoer te bevestigen.',
    ...
    // new user:
    'welcome'                                 => 'Welkom bij '.config('app.name').'!',
    'submit'                                  => 'Invoeren',
    'submit_yes_really'                       => 'Invoeren (ik weet wat ik doe)',
    'getting_started'                         => 'Aan de start!',
    'to_get_started'                          => 'Het is goed om te zien dat de installatie van '.config('app.name').' gelukt is. Voer de naam van je bank in en het saldo van je belangrijkste betaalrekening. Meerdere rekeningen kan je later toevoegen, maar we moeten ergens beginnen natuurlijk.',
    'savings_balance_text'                    => config('app.name').' maakt automatisch een spaarrekening voor je. Daar zit normaal geen geld in, maar je kan hier opgeven hoeveel er op staat.',
    'finish_up_new_user'                      => 'That\'s it! Druk op <strong>Opslaan</strong> om door te gaan. Je gaat dan naar de homepage van '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'Hoera! Je nieuwe accounts zijn opgeslagen.',
    'set_preferred_language'                  => 'Als je '.config('app.name').' liever in een andere taal gebruikt, geef dat dan hier aan.',
    'language'                                => 'Taal',
    ...
    'import_index_title'                    => 'Transacties importeren in '.config('app.name'),
    'import_data'                           => 'Importeer data',
    'import_transactions'                   => 'Importeer transacties',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Deze functie werkt niet als je '.config('app.name').' gebruikt in combinatie met Sandstorm.IO.',
    ...
    'no_accounts_imperative_asset'          => 'Om '.config('app.name').' te gebruiken moet je minstens één betaalrekening hebben. Die kan je nu maken:',
    ...
    'no_recurring_intro_default'            => 'Je hebt nog geen periodieke transacties. Je kan deze gebruiken om er voor te zorgen dat '.config('app.name').' automatisch nieuwe transacties voor je maakt.',
    ...
    'warning_duplicates_repetitions'        => 'Soms zie je hier datums dubbel staan. Dat kan als meerdere herhalingen in elkaars vaarwater zitten. '.config('app.name').' maakt altijd maar één transactie per dag.',
    ...
    'recurring_never_cron'                  => 'Het lijkt er op dat je cronjob die nodig is voor periodieke transacties nog nooit gedraaid heeft. Niet zo gek natuurlijk als je '.config('app.name').' echt net geïnstalleerd hebt, maar denk eraan dat je dit regelt. Check de helppagina\'s via het (?)-icoontje rechtsboven.',
    ...
    'help_first_date_no_past'            => 'Geef aan wanneer je de eerste transactie verwacht. '.config('app.name').' zal geen transacties in het verleden maken.',
    ...
    'help_weekend'                       => 'Wat moet '.config('app.name').' doen als de periodieke transactie in het weekend valt?',
