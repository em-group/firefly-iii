...
return [
    ...
    'index'                  => 'Bienvenue chez <strong>'.config('app.name').'</strong> ! Sur cette page, vous obtenez un aperçu rapide de vos finances. Pour plus d’informations, consultez comptes &rarr; <a href=":asset">Comptes d’actif</a> et, bien sûr, les pages des <a href=":budgets">Budgets</a> et des <a href=":reports">rapports</a>. Ou juste jetez un coup d’œil et voyez où vous vous retrouvez.',
    ...
    'reports-index-start'    => config('app.name').' prend en charge un certain nombre de types de rapports. Lisez-les en cliquant sur l\'icône <i class="fa fa-question-circle"></i> dans le coin supérieur droit.',
    ...
    'currencies-index'       => config('app.name').' prend en charge plusieurs devises. Bien que l\'Euro soit la devise par défaut, cette dernière peut être changée pour le Dollar américain et de nombreuses autres devises. Comme vous pouvez le remarquer une petite sélection des monnaies a été incluse, mais vous pouvez ajouter vos propres devises si vous le souhaitez. Gardez à l\'esprit que la modification de la devise par défaut ne modifie pas la monnaie des transactions existantes : '.config('app.name').' prend en charge l’utilisation de plusieurs devises en même temps.',
