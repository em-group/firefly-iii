...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Si vous exécutez votre propre instance de '.config('app.name').', vérifiez les logs dans <code>storage/logs</code> pour obtenir des instructions.',
    ...
    'need_more_help'                             => 'Si vous désirez plus de renseignements sur l\'utilisation de '.config('app.name').', merci <a href="https://github.com/firefly-iii/firefly-iii/issues">d\'ouvrir un ticket sur Github</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' n\'est pas en mesure de vous rediriger vers la bonne page. Veuillez nous en excuser.',
    ...
    'invalid_locale_settings'                    => config('app.name').' ne parvient pas à formater les montants monétaires car il manque des paquets logiciels sur votre serveur. Voici <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">des instructions sur comment procéder</a>.',
    ...
    'login_page_title'                           => 'Connectez-vous à '.config('app.name'),
    'register_page_title'                        => 'Enregistrez-vous à '.config('app.name'),
    'forgot_pw_page_title'                       => 'Mot de passe '.config('app.name').' oublié',
    'reset_pw_page_title'                        => 'Réinitialiser votre mot de passe '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version demande d\'autorisation',
    ...
    'admin_update_check_explain'                 => config('app.name').' peut vérifier les mises à jour automatiquement. Lorsque vous activez ce paramètre, il contacte Github pour voir si une nouvelle version de '.config('app.name').' est disponible. Quand c\'est le cas, vous recevrez une notification. Vous pouvez tester cette notification en utilisant le bouton sur la droite. Veuillez indiquer ci-dessous si vous voulez que '.config('app.name').' vérifie les mises à jour.',
    'check_for_updates_permission'               => config('app.name').' peut vérifier les mises à jour, mais il a besoin de votre permission pour le faire. Veuillez vous rendre à l\'adresse <a href=":link">administration</a> pour indiquer si vous souhaitez que cette fonctionnalité soit activée.',
    ...
    'admin_update_check_now_explain'             => 'Si vous appuyez sur le bouton, '.config('app.name').' verra si votre version actuelle est la dernière.',
    'check_for_updates_button'                   => 'Vérifier maintenant !',
    'update_new_version_alert'                   => 'Une nouvelle version de '.config('app.name').' est disponible. Vous utilisez v:your_version, la dernière version est v:new_version qui a été publiée le :date.',
    ...
    'search_found_transactions'                  => config('app.name').' a trouvé :count opération(s) en :time secondes.',
    'search_for_query'                           => config('app.name').' recherche des opérations contenant tous ces mots : <span class="text-info">:query</span>',
    ...
    'search_box_intro'                           => 'Bienvenue à la fonction de recherche de '.config('app.name').'. Entrez votre requête de recherche dans la zone. Assurez-vous de consulter le fichier d’aide parce que la recherche est assez avancée.',
    ...
    'export_data_intro'                          => 'Utilisez les données exportées pour passer à une nouvelle application financière. Veuillez noter que ces fichiers ne sont pas destinés à être sauvegardés. Ils ne contiennent pas assez de métadonnées pour restaurer complètement une nouvelle installation de '.config('app.name').'. Si vous souhaitez effectuer une sauvegarde de vos données, veuillez sauvegarder la base de données directement.',
    'export_format'                              => 'Format d\'export',
    'export_format_csv'                          => 'Valeurs séparées par des virgules (fichier CSV)',
    'export_format_mt940'                        => 'Format compatible MT940',
    'include_old_uploads_help'                   => config('app.name').' ne détruit pas les fichiers CSV originaux que vous avez déjà importé dans le passé. Vous pouvez les inclure dans votre exportation.',
    'do_export'                                  => 'Exporter',
    ...
    'instructions_rule_from_bill'                => 'Afin d’associer des transactions à votre nouvelle facture « :name », '.config('app.name').' peut créer une règle qui sera automatiquement appliquée sur toutes les transactions déjà enregistrées. Veuillez vérifier les détails ci-après et enregistrer la règle pour que '.config('app.name').' associe automatiquement des transactions à votre nouvelle facture.',
    ...
    'apply_rule_group_selection_intro'           => 'Les groupes de règles comme ":title" ne s\'appliquent normalement qu\'aux transactions nouvelles ou mises à jour, mais vous pouvez dire à '.config('app.name').' d\'exécuter toutes les règles de ce groupe sur une sélection de vos transactions existantes. Cela peut être utile lorsque vous avez mis à jour un groupe de règles et avez besoin que les modifications soient appliquées à l’ensemble de vos autres transactions.',
    ...
    'create_rule_for_bill_txt'                => 'Vous venez de créer une nouvelle facture appelée ":name", félicitations ! '.config('app.name').' peut automatiquement faire correspondre de nouvelles dépenses à cette facture. Par exemple, chaque fois que vous payez votre loyer, la facture "loyer" sera liée à la dépense. De cette façon, '.config('app.name').' peut vous montrer exactement quelles factures sont dues et lesquelles ne le sont pas. Pour ce faire, une nouvelle règle doit être créée. '.config('app.name').' a rempli certaines valeurs qui doivent être logiques pour vous. Veuillez vous assurer qu\'elles sont correctes. Si c\'est le cas, '.config('app.name').' liera automatiquement la bonne dépense à la bonne facture. S\'il vous plaît vérifier également les déclencheurs pour voir s\'ils sont corrects, en ajouter ou les corriger si nécessaire.',
    ...
    'pref_languages_help'                     => config('app.name').' prend en charge plusieurs langues. Laquelle préférez-vous ?',
    ...
    'delete_your_account_help'                => 'La suppression de votre compte supprimera également les comptes, les opérations, <em>tout</em> ce que vous pourriez avoir enregistré dans '.config('app.name').'. Tout sera SUPPRIME.',
    ...
    'secure_pw_ff'                            => 'Vous utilisez le même mot de passe partout sur Internet ? Si un seul site se fait dérober ce mot de passe, c\'est l\'ensemble de vos comptes et données qui sont exposées aux pirates. '.config('app.name').' compte sur vous pour choisir un mot de passe unique et fort pour protéger vos informations financières.',
    'secure_pw_check_box'                     => 'Pour vous y aider '.config('app.name').' peut vérifier que le mot de passe que vous souhaitez utiliser n\'a jamais été dérobé auparavant. Si c\'est le cas, '.config('app.name').' vous conseille de NE PAS utiliser ce mot de passe.',
    'secure_pw_working_title'                 => 'Comment est-ce que cela fonctionne ?',
    'secure_pw_working'                       => 'Si vous cochez l\'option, '.config('app.name').' enverra les 5 premiers caractères de l\'empreinte SHA1 de votre mot de passe au <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">site Web de Troy Hunt</a> pour contrôler si elle y apparait. Cela vous empêchera d\'utiliser des mots de passe non sûrs tel que recommandé par la dernière <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">publication spéciale NIST</a> sur le sujet.',
    ...
    'delete_local_info_only'                  => 'Comme vous vous authentifiez via ":login_provider", cela ne supprimera que les informations locales de '.config('app.name').'.',
    ...
    'cannot_disable_currency'                 => 'Impossible de désactiver :name parce qu’il est encore utilisé.',
    ...
    'currencies_intro'                        => config('app.name').' prend en charge diverses devises que vous pouvez configurer et activer ici.',
    ...
    'auto_match_on'                           => 'Automatiquement mis en correspondance par '.config('app.name'),
    'auto_match_off'                          => 'Pas mis automatiquement en correspondance par '.config('app.name'),
    ...
    // accounts:
    'account_missing_transaction'             => 'Le compte #:id (":name") ne peut pas être consulter directement et '.config('app.name').' ne dispose d\'aucune information pour vous rediriger.',
    ...
    'reconcile_has_more'                      => 'Votre registre '.config('app.name').' a plus d\'argent que ce que votre banque prétend que vous devriez avoir. Veuillez choisir comment gérer la situation parmi les différentes possibilités offertes. Ensuite, appuyez sur "Confirmer le rapprochement".',
    'reconcile_has_less'                      => 'Votre registre '.config('app.name').' a moins d\'argent que ce que votre banque prétend que vous devriez avoir. Veuillez choisir comment gérer la situation parmi les différentes possibilités offertes. Ensuite, appuyez sur "Confirmer le rapprochement".',
    'reconcile_is_equal'                      => 'Votre registre '.config('app.name').' et vos relevés bancaires correspondent. Il n\'y a rien à faire. Appuyez sur "Confirmer le rapprochement" pour confirmer votre entrée.',
    ...
    // new user:
    'welcome'                                 => 'Bienvenue sur '.config('app.name').' !',
    'submit'                                  => 'Soumettre',
    'submit_yes_really'                       => 'Soumettre (je sais ce que je fais)',
    'getting_started'                         => 'Mise en route',
    'to_get_started'                          => 'Vous venez d\'installer '.config('app.name').' avec succès. Pour commencer avec cet outil, entrez le nom de votre banque et le solde de votre compte courant principal. Ne vous inquiétez pas si vous avez plusieurs comptes. Vous pourrez les ajouter plus tard. '.config('app.name').' a simplement besoin de quelque chose pour commencer.',
    'savings_balance_text'                    => config('app.name').' créera automatiquement un compte d\'épargne pour vous. Par défaut, il n\'y aura pas d\'argent sur ce compte d\'épargne, mais si vous indiquez un montant à '.config('app.name').', il l\'enregistrera en tant que solde.',
    'finish_up_new_user'                      => 'C\'est tout ! Vous pouvez continuer en appuyant sur <strong>Envoyer</strong>. Vous passerez à l\'index de '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'Super ! Vos nouveaux comptes ont été créés.',
    'set_preferred_language'                  => 'Si vous préférez utiliser '.config('app.name').' dans une autre langue, veuillez l\'indiquer ici.',
    'language'                                => 'Langue',
    ...
    'import_index_title'                    => 'Importer des opérations dans '.config('app.name'),
    'import_data'                           => 'Importer des données',
    'import_transactions'                   => 'Importer des opérations',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Cette fonction n\'est pas disponible lorsque vous utilisez '.config('app.name').' dans un environnement Sandstorm.io.',
    ...
    'no_accounts_imperative_asset'          => 'Pour commencer à utiliser '.config('app.name').', vous devez créer au moins un compte d\'actif. Faisons-le maintenant :',
    ...
    'no_recurring_intro_default'            => 'Vous n’avez pas encore d\'opérations périodiques. Vous pouvez en utiliser pour que '.config('app.name').' crée automatiquement des opérations pour vous.',
    ...
    'warning_duplicates_repetitions'        => 'Dans de rares cas, des dates apparaissent en double dans cette liste. Cela peut arriver lorsque plusieurs répétitions se superposent. '.config('app.name').' génèrera toujours une seule transaction par jour.',
    ...
    'recurring_never_cron'                  => 'Il semble que la tâche cron qui supporte les opérations périodiques n\'a jamais été exécutée. Ceci est normal si vous venez juste d\'installer '.config('app.name').', mais pensez à la configurer dès que possible. Veuillez consulter la page d\'aide associée en cliquant sur l\'icône (?) en haut à droite de la page.',
    ...
    'help_first_date_no_past'            => 'Indiquer la première occurrence attendue. '.config('app.name').' ne créera pas d\'opérations dans le passé.',
    ...
    'help_weekend'                       => 'Que doit faire '.config('app.name').' lorsque l’opération périodique tombe un samedi ou un dimanche ?',
