...
return [
    ...
    'two_factor_lost_fix_self'                   => '若您自行架構 '.config('app.name').'，請查閱 <code>storage/logs</code> 中日誌檔內的指示。',
    ...
    'need_more_help'                             => '如果您需要更多 '.config('app.name').' 的協助，請 <a href="https://github.com/firefly-iii/firefly-iii/issues">於 GitHub 建立提問</a>。',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' 無法重新導向正確頁面，抱歉。',
    ...
    'invalid_locale_settings'                    => config('app.name').' 無法格式化金額，因為伺服器缺少必要套件。請參閱 <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">相關指示</a>。',
    'quickswitch'                                => '快速切換',
    ...
    'reset_button'                               => '重設',
    'want_to_login'                              => '我想登入',
    'login_page_title'                           => '登入 '.config('app.name'),
    'register_page_title'                        => '在 '.config('app.name').' 註冊',
    'forgot_pw_page_title'                       => '忘記您 '.config('app.name').' 的密碼',
    'reset_pw_page_title'                        => '重設您 '.config('app.name').' 的密碼',
    ...
    'expected_total'                             => '預期總數',
    // API access
    'authorization_request'                      => config('app.name').' :version 版授權請求',
    ...
    'admin_update_check_explain'                 => config('app.name').' 可以自動檢查更新。啟用此設定時，將會自動連接 Github 查看是否有新版本可用，並在可用時顯示一則通知。您可使用右方按鈕測試通知功能，並於下方表示您是否希望 '.config('app.name').' 檢查更新。',
    'check_for_updates_permission'               => config('app.name').' 可以自動檢查更新，但需要您先許可。請前往 <a href=":link">系統管理</a> 表示您是否想啟用此功能。',
    'updates_ask_me_later'                       => '稍後再詢問',
    'updates_do_not_check'                       => '不檢查更新',
    'updates_enable_check'                       => '啟用更新檢查',
    'admin_update_check_now_title'               => '立刻檢查更新',
    'admin_update_check_now_explain'             => '如果您按下此按鈕，'.config('app.name').' 將會檢查您現在的版本是否為最新版。',
    'check_for_updates_button'                   => '馬上檢查！',
    'update_new_version_alert'                   => '較新版本的 '.config('app.name').' 已可使用，您目前為 :your_version 版，最新版本為 :new_version 版，於 :date 釋出。',
    ...
    'search'                                     => '搜尋',
    'search_query'                               => '查詢',
    'search_found_transactions'                  => config('app.name').' 找到 :count 筆交易，花了 :time 秒。',
    'search_for_query'                           => config('app.name').' 正搜尋包含所有這些字詞的交易：<span class="text-info">:query</span>',
    ...
    'search_box'                                 => '搜尋',
    'search_box_intro'                           => '歡迎使用 '.config('app.name').' 搜尋功能，請於方框內鍵入搜尋條件。搜尋功能相當厲害，記得查看說明檔。',
    ...
    'export_data_intro'                          => '使用匯出資料轉移至新的財務應用程式。請注意這些檔案並非備份，它們未包含足夠的中繼資料在全新的 '.config('app.name').' 重新還原。如果您希望備份資料，請直接備份整個資料庫。',
    'export_format'                              => '匯出格式',
    'export_format_csv'                          => '逗號分隔數值 (CSV 檔案)',
    'export_format_mt940'                        => 'MT940 相容格式',
    'include_old_uploads_help'                   => config('app.name').' 不會扔掉過去已匯入的原始 CSV 檔，你可以將它們包含在匯出的檔案中。',
    'do_export'                                  => '匯出',
    ...
    'instructions_rule_from_bill'                => '為了媒合交易至您的新帳單 ":name"，'.config('app.name').' 可以建立一個針對您儲存的每一筆交易且自動啟動的規則。請驗證下方的資訊並儲存規則，讓 '.config('app.name').' 自動媒合交易至您的新帳單。',
    'rule_is_strict'                             => '嚴謹規則',
    ...
    'apply_rule_selection_intro'                 => '規則如 ":title" 一般僅套用至新的或更新後的交易，但您可要求 '.config('app.name').' 針對既有的單筆或多筆交易執行規則。在您更新一則規則後，且必須套用該規則至其他交易時，即可使用此功能。',
    ...
    'apply_rule_group_selection_intro'           => '規則群組如 ":title" 一般僅套用至新的或更新後的交易，但您可要求 '.config('app.name').' 針對既有的單筆或多筆交易執行規則群組內的規則。在您更新一則規則群組後，且必須套用該群組至其他交易時，即可使用此功能。',
    ...
    'create_rule_for_bill_txt'                => '恭喜，您方才建立了名為 ":name" 新帳單！'.config('app.name').' 可魔幻自動地媒合新提款至此帳單。舉例而言，無論何時您付了房租，帳單 "房租" 會連結到該支出。依此，'.config('app.name').' 可以準確的顯示哪些帳單即期又哪些尚未。若要使用此功能，必須建立一個新的規則。'.config('app.name').' 已為您填寫一些合理的預設資料，請確認是否正確。若然，'.config('app.name').' 將自動連結正確的提款至正確的帳單，請檢查觸發器並確認無誤，若有錯誤請另行增添。',
    ...
    'pref_1Y'                                 => '1年',
    'pref_languages'                          => '語言',
    'pref_languages_help'                     => config('app.name').' 支援多種語言，您想顯示哪一種？',
    ...
    'delete_your_account_help'                => '刪除您的帳號，將一併刪除任何帳戶、交易，<em>一切</em> 您可能儲存在 '.config('app.name').' 上的資料，都將<strong>一去不復返</strong>。',
    ...
    'secure_pw_ff'                            => '您在網路上都用同一個密碼走天涯嗎？如果一個網站遺失您的密碼，駭客就可存取您所有的資料。'.config('app.name').' 倚賴您選擇一個強而獨特的密碼以保護您的財務紀錄。',
    'secure_pw_check_box'                     => '為此，'.config('app.name').' 可協助您檢查選用的密碼過去是否曾被竊取。若然如此，'.config('app.name').' 建議您<strong>避免</strong>使用此密碼。',
    'secure_pw_working_title'                 => '這是如何運作？',
    'secure_pw_working'                       => '若您勾選此方塊，'.config('app.name').' 將以您的密碼產生一個 SHA1 湊合，並將首 5 個字元傳送到 <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">Troy Hunt 的網站</a> 比對一個清單，避免您使用一些已知不安全的密碼，遵從最近 <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">NIST Special Publication</a> 有關此課題的建議。',
    ...
    'email_verification'                      => '將同時寄送一封電子郵件到您的舊<strong>和</strong>新的電子郵件地址。顧及安全考慮，在驗證新的電子郵件地址之前，您將無法登入。如果您不確定您的 '.config('app.name').' 安裝是否能夠發送電子郵件，請勿使用此功能。如果您是管理員，可以在 <a href="/admin">系統管理</a> 中對此進行測試。',
    ...
    'delete_local_info_only'                  => '由於您以 ":login_provider" 驗證，僅會刪除本機 '.config('app.name').' 資訊。',
    ...
    'currencies_intro'                        => config('app.name').' 支援多種貨幣，您可在此設定並啟用。',
    ...
    'auto_match_on'                           => '由 '.config('app.name').' 自動配對',
    'auto_match_off'                          => '不由 '.config('app.name').' 自動配對',
    ...
    'reconcile_has_more'                      => '您的 '.config('app.name').' 帳目比您的銀行有更多的金錢，有多個選項可應處，請選擇並按下 "確認對帳"。',
    'reconcile_has_less'                      => '您的 '.config('app.name').' 帳目比您的銀行有更少的金錢，有多個選項可應處，請選擇並按下 "確認對帳"。',
    'reconcile_is_equal'                      => '您的 '.config('app.name').' 帳目和您的銀行相符，無須處理，請按下 "確認對帳" 確認匯入。',
    ...
    // new user:
    'welcome'                                 => '歡迎使用 '.config('app.name').'！',
    'submit'                                  => '送出',
    'submit_yes_really'                       => '送出 (我知道我在幹嘛)',
    'getting_started'                         => '開始使用',
    'to_get_started'                          => '很高興看到您成功安裝 '.config('app.name').'。要開始使用此工具，請輸入您主要使用的的銀行名稱與餘額。別擔心，如您有多個帳戶，可以稍後新增，這儘是為了 '.config('app.name').' 需要初始使用而設。',
    'savings_balance_text'                    => config('app.name').' 會自動為您建立儲蓄帳戶。預設下，儲蓄帳戶會沒有金錢，但若您告訴 '.config('app.name').' 餘額為何，亦可以此儲存。',
    'finish_up_new_user'                      => '這就對了！您可按下 <strong>送出</strong> 繼續，您將被帶至 '.config('app.name').' 的首頁。',
    'stored_new_accounts_new_user'            => '太好了！您的帳戶已被儲存。',
    'set_preferred_language'                  => '若您傾向在 '.config('app.name').' 使用其他語言，請於此表示。',
    'language'                                => '語系',
    ...
    'firefly_instance_configuration'        => config('app.name').' 的組態設定選項',
    'setting_single_user_mode'              => '單使用者模式',
    'setting_single_user_mode_explain'      => '預設下，'.config('app.name').' 只接受一個 (1) 註冊，即您本人。此為安全措施，在除經你同意外，防止其他人使用您的空間，目前註冊功能已關閉。當您取消此核選方塊後，若他們能連接到此空間 (在已聯網狀態)，其他人也可以使用。',
    ...
    'sandstorm_not_available'               => '當您在 Sandstorm.io 環境使用 '.config('app.name').' 時，此功能不可用。',

    // empty lists? no objects? instructions:
    'no_accounts_title_asset'               => '建立新資產帳戶',
    'no_accounts_intro_asset'               => '您還沒有資產帳戶。資產帳戶可以包括: 你的支票帳戶、儲蓄帳戶、共用帳戶甚至信用卡。',
    'no_accounts_imperative_asset'          => '要使用 '.config('app.name').' ，您至少需要一个資產帳戶。現在開始吧：',
    ...
    'no_recurring_intro_default'            => '您目前沒有週期性交易。您可使用此功能讓 '.config('app.name').' 自動為您建立交易。',
    ...
    'warning_duplicates_repetitions'        => '在少數情況下，日期在此清單上會顯示兩次，這在多個重複衝突之下會發生。'.config('app.name').' 每天只會產生一條紀錄。',
    ...
    'recurring_never_cron'                  => '用來支援週期性交易的 cron job 似乎沒有運行過，這在您剛安裝 '.config('app.name').' 沒多久時是非常正常的，但能越快處理越好。請使用本頁右上角的 (?)-圖示查閱說明頁面。',
    ...
    'help_first_date_no_past'            => '表示第一筆預期的週期交易，'.config('app.name').' 不會建立過去的交易。',
    'no_currency'                        => '(沒有貨幣)',
    ...
    'help_weekend'                       => '當週期性交易落於週六及週日時， '.config('app.name').' 該如何應處？',
    'do_nothing'                         => '仍建立交易',
