...
return [
    ...
    'index'                  => '歡迎使用 <strong>'.config('app.name').'</strong>！此頁可讓您快速概覽財務狀況。至於詳細資料，可見 帳戶 &rarr; <a href=":asset">資產帳戶</a>，另見 <a href=":budgets">預算</a> 和 <a href=":reports">報表</a> 頁面。當然，您也可以到處逛逛看。',
    ...
    'reports-index-start'    => config('app.name').' 支援數種不同類型的報表，按一下右上方的 <i class="fa fa-question-circle"></i> 圖示可查看更多資訊。',
    'reports-index-examples' => '舉例說：<a href=":one">月財務概覽</a>、<a href=":two">年度財務概覽</a> 及 <a href=":three">預算概覽</a>，記得去看看。',
    'currencies-index'       => config('app.name').' 支援多種貨幣，預設為歐元，但亦可設成美元或其他貨幣。您可見到系統已預設包含一些貨幣種類，但您也可自行新增其他貨幣。修改預設貨幣並不會改變現有交易的貨幣種類：'.config('app.name').' 是支援同時使用多種貨幣的。',
    'transactions-index'     => '這些支出、存款與轉帳談不上別出心裁：這些範例是自動產生的。',
    'piggy-banks-index'      => '您可見到有 3 個小豬撲滿。使用 + 號、- 號按鈕控制每個小豬撲滿的存款額，按一下小豬撲滿的名稱則可查看管理詳情。',
    'import-index'           => '任何 CSV 格式的檔案都可匯入 '.config('app.name').'，也支援自 bunq 與 Spectre 匯入資料，日後或會支援其他銀行與金融機構。展示使用者只會看到「虛擬」提供者的示範，系統會隨機產生交易紀錄以示範操作過程。',
