...
',
    ...
    'two_factor_lost_fix_self'                   => 'Kendi '.config('app.name').' örneğinizi çalıştırıyorsanız, talimatlar için <code>storage/logs</code>\'daki günlükleri kontrol edin.',
    ...
    'need_more_help'                             => config('app.name').' kullanımında daha fazla yardıma ihtiyacınız olursa, lütfen <a href="https://github.com/firefly-iii/firefly-iii/issues">Github\'da bir talep açın</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' sizi doğru sayfaya yönlendiremiyor. Özür dileriz.',
    ...
    'invalid_locale_settings'                    => config('app.name').' parasal tutarları biçimlendiremiyor çünkü gerekli paketler sunucunuzda yok. <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">Bunun nasıl yapıldığıyla ilgili talimatlar</a> var.',
    ...
    'login_page_title'                           => config('app.name').'\'e giriş yapın',
    'register_page_title'                        => config('app.name').'\'e kayıt olun',
    'forgot_pw_page_title'                       => config('app.name').' için şifrenizi mi unuttunuz',
    'reset_pw_page_title'                        => config('app.name').' şifrenizi sıfırlayın',
    ...
    // API access
    'authorization_request'                      => config('app.name').' v: version Yetkilendirme İsteği',
    ...
    'admin_update_check_explain'                 => config('app.name').' güncellemeleri otomatik olarak kontrol edebilir. Bu ayarı etkinleştirdiğinizde, '.config('app.name').'\'ün yeni bir sürümünün mevcut olup olmadığını görmek için Github ile iletişime geçecektir. Bu olduğunda, bir bildirim alacaksınız. Bu bildirimi sağdaki düğmeyi kullanarak test edebilirsiniz. '.config('app.name').'\'ün güncellemeleri kontrol etmesini istiyorsanız lütfen aşağıya belirtin.',
    'check_for_updates_permission'               => config('app.name').' güncellemeleri kontrol edebilir, ancak bunu yapmak için izniniz gerekir. Bu özelliğin etkinleştirilmesini isteyip istemediğinizi belirtmek için lütfen <a href=":link"> yönetim </a> bölümüne gidin.',
    ...
    'admin_update_check_now_explain'             => 'Düğmeye basarsanız, '.config('app.name').' geçerli sürümünüzün en son olup olmadığını görür.',
    'check_for_updates_button'                   => 'Kontrol Et!',
    'update_new_version_alert'                   => config('app.name').'\'ün yeni bir sürümü mevcut. v:your_version  çalıştırıyorsunuz, en son sürüm v:new_version  :date. tarihinde yayınlandı.',
...
',
    ...
    'search'                                     => 'Ara',
    'search_query'                               => 'Sorgu',
    'search_found_transactions'                  => config('app.name').' :time saniyede :count hesap haraketi buldu.',
    'search_for_query'                           => config('app.name').' şu kelimelerin hepsini içeren hareketleri arıyor: <span class="text-info">:query</span>',
    ...
    'search_box'                                 => 'Ara',
    'search_box_intro'                           => config('app.name').'\'ün arama fonksiyonuna hoş geldiniz. Kutuya arama sorgunuzu giriniz. Arama oldukça gelişmiş olduğundan yardım dosyasını kontrol ettiğinizden emin olun.',
    ...
    'include_old_uploads_help'                   => config('app.name').' geçmişte aktarmış olduğunuz CSV dosyalarınızı herhangi bir yere atmıyor. Onları ihracata ekleyebilirsiniz.',
    ...
    'instructions_rule_from_bill'                => 'Hesap hareketlerini ":name" ile eşleştirebilmeniz için '.config('app.name').' depolayacağınız tüm hesap hareketlerine karşı otomatik olarak kontrol edebilen bir kural oluşturabilir. Lütfen aşağıdaki detayları doğrulayın ve '.config('app.name').'\'ün hesap hareketlerini yeni faturanızla otomatik olarak eşleştirebilmesi için bu kuralı depolayın.',
...
işlemlerin kontrol edildiğini lütfen unutmayın.',
    ...
    'apply_rule_selection_intro'                 => '":title" gibi kurallar normalde sadece yeni ve güncellenen işlemlerde geçerlidir ama '.config('app.name').'\'e onları mevcut işlemlerinizin istediğiniz bölümlerinde uygulanmasını söyleyebilirsiniz. Bu bir kuralı değiştirdiğinizde ve bunun diğer tüm işlemlerde uygulanmasını istediğinizde yararlı olabilir.',
    ...
    'apply_rule_group_selection_intro'           => '":title" gibi kural grupları normalde sadece yeni ve güncellenen işlemlerde geçerlidir ama '.config('app.name').'\'e onları mevcut işlemlerinizin istediğiniz bölümlerinde uygulanmasını söyleyebilirsiniz. Bu bir kural grubunu değiştirdiğinizde ve bunun diğer tüm işlemlerde uygulanmasını istediğinizde yararlı olabilir.',
    ...
    'pref_languages_help'                     => config('app.name').' birçok dili destekliyor. Hangisini tercih edersiniz?',
    ...
    'delete_your_account_help'                => 'Hesabınızı silmeniz aynı zamanda '.config('app.name').'\'e kaydetmiş olabileceğiniz hesapları, işlemleri, <em>her şeyi</em> de silecektir. YOK OLACAK.',
    ...
    'currencies_intro'                        => config('app.name').', burada ayarlayabileceğiniz ve etkinleştirebileceğiniz çeşitli para birimlerini destekliyor.',
    ...
    'reconcile_has_more'                      => config('app.name').' hesabınızda bankanızın iddia ettiğinden daha fazla para var. Birkaç seçenek var. Lütfen ne yapılacağını seçin. Daha sonra "Uzlaşmayı onayla" ya basın.',
    'reconcile_has_less'                      => config('app.name').'\'de bankanızın iddia ettiğinden daha az para var. Birkaç seçenek var. Lütfen ne yapılacağını seçin. Daha sonra "Uzlaşmayı onayla"\'ya basın.',
    'reconcile_is_equal'                      => config('app.name').' hesabınızdaki para ile banka ekstreniz eşleşiyor. Yapılacak bir şey yok. Lütfen girdinizi onaylamak için "Uzlaşmayı onayla"\'ya basın.',
    ...
    // new user:
    'welcome'                                 => config('app.name').'\'e hoşgeldiniz!',
    'submit'                                  => 'Gönder',
    'submit_yes_really'                       => 'Submit (I know what I\'m doing)',
    'getting_started'                         => 'Başla',
    'to_get_started'                          => config('app.name').'\'ü başarılı şekilde yüklediğinizi görmek güzel. Bu aracı kullanmak için lütfen banka adınızı ve ana hesabınızın bakiyesini girin. Birden fazla hesabınız varsa endişelenmeyin. Onları daha sonra ekleyebilirsiniz. Bu adım sadece '.config('app.name').'\'ün bir yerden başlaması gerektiği içindir.',
    'savings_balance_text'                    => config('app.name').' sizin için otomatik olarak bir birikim hesabı oluşturacaktır. Varsayılan olarak birikim hesabınızda hiç para olmayacaktır ama '.config('app.name').'\'e dengelemesini söylerseniz o şekilde saklayacaktır.',
    'finish_up_new_user'                      => 'İşte bu! <strong>Submit</strong> tıklayarak devam edebilirsiniz. '.config('app.name').' Anasayfasına yönlendirileceksiniz.',
    ...
    'firefly_instance_configuration'        => config('app.name').' için yapılandırma seçenekleri',
    'setting_single_user_mode'              => 'Tek kullanıcı modu',
    'setting_single_user_mode_explain'      => 'Varsayılan olarak '.config('app.name').' sadece bir kayıt kabul eder: sizi. Bu başkalarının sizin isteklerinizi kontrol etmemesi için bir güvenlik önlemidir. Gelecekteki kayıtlar engellendi. Bu kutunun işaretini kaldırdığınızda erişebildiklerini var sayarsak (internete bağlı olduğunda) başkaları da sizin isteklerinizi kullanabilir.',
    ...
    'import_index_title'                    => config('app.name').'\'e veri aktarma',
    'import_data'                           => 'Veriyi içe aktar',
    'import_transactions'                   => 'İşlemleri içe aktarma',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Bir Sandstorm.io ortamında '.config('app.name').' kullanıyorsanız, bu işlev kullanılamaz.',
    ...
    'no_accounts_imperative_asset'          => config('app.name').' kullanmaya başlamak için en az bir varlık hesabı açmanız gerekir. Hadi şimdi yapalım:',
    ...
    'no_recurring_intro_default'            => 'Henüz yinelenen işleminiz yok. '.config('app.name').'\'ün sizin için otomatik olarak işlemler oluşturmasını sağlamak için bunları kullanabilirsiniz.',
