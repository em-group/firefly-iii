...
return [
    ...
    'two_factor_lost_fix_self'                   => 'Se você executar sua própria instância do '.config('app.name').', verifique os logs no <code>storage/logs</code> para obter instruções.',
    ...
    'need_more_help'                             => 'Se você precisa de mais ajuda usando o '.config('app.name').', por favor <a href="https://github.com/firefly-iii/firefly-iii/issues">abra um ticket no Github</a>.',
    ...
    'cannot_redirect_to_account'                 => config('app.name').' não pode redirecioná-lo para a página correta. Minhas desculpas.',
    ...
    'invalid_locale_settings'                    => 'O '.config('app.name').' não consegue formatar quantidades monetárias porque está faltando os pacotes necessários no seu servidor. Existem <a href="https://github.com/firefly-iii/help/wiki/Missing-locale-packages">instruções de como fazer isso</a>.',
    ...
    'login_page_title'                           => 'Login no '.config('app.name'),
    'register_page_title'                        => 'Registrar no '.config('app.name'),
    'forgot_pw_page_title'                       => 'Esqueceu sua senha do '.config('app.name'),
    'reset_pw_page_title'                        => 'Redefinir sua senha para '.config('app.name'),
    ...
    // API access
    'authorization_request'                      => config('app.name').' v:version Pedido de autorização',
    ...
    'admin_update_check_explain'                 => 'O '.config('app.name').' pode verificar atualizações automaticamente. Quando você habilita essa configuração, contatará Github para ver se uma nova versão do '.config('app.name').' está disponível. Quando for, você receberá uma notificação. Você pode testar esta notificação usando o botão à direita. Indique abaixo se você deseja que o '.config('app.name').' verifique se há atualizações.',
    'check_for_updates_permission'               => 'O '.config('app.name').' pode verificar atualizações, mas precisa da sua permissão para fazê-lo. Acesse o <a href=":link">administração</a> para indicar se você gostaria que esse recurso fosse ativado.',
    ...
    'admin_update_check_now_explain'             => 'Se você pressionar o botão, o '.config('app.name').' verá se sua versão atual é a mais recente.',
    'check_for_updates_button'                   => 'Verifique agora!',
    'update_new_version_alert'                   => 'Uma nova versão do '.config('app.name').' está disponível. Você está executando v:your_version, a versão mais recente é v:new_version que foi lançada em :date.',
    ...
    'search'                                     => 'Pesquisa',
    'search_query'                               => 'Pedido',
    'search_found_transactions'                  => config('app.name').' encontrou :count transação(ões) em :time segundo(s).',
    'search_for_query'                           => config('app.name').' está procurando transações com todas estas palavras neles: <span class="text-info">:query</span>',
    ...
    'search_box_intro'                           => 'Bem-vindo à função de pesquisa do '.config('app.name').'. Digite sua consulta de pesquisa na caixa. Certifique-se de verificar o arquivo de ajuda porque a pesquisa é bastante avançada.',
    ...
    'export_data_intro'                          => 'Use os dados exportados para mover para uma nova aplicação financeira. Observe que esses arquivos não são feitos como um backup. Eles não contêm informações suficientes sobre os dados para restaurar completamente uma nova instalação do '.config('app.name').'. Se você quiser fazer um backup de seus dados, faça backup do banco de dados diretamente.',
    'export_format'                              => 'Formato de exportação',
    'export_format_csv'                          => 'Valores separados por vírgula (arquivo CSV)',
    'export_format_mt940'                        => 'Compatível com formato MT940',
    'include_old_uploads_help'                   => 'O '.config('app.name').' não joga fora os arquivos CSV originais que você importou no passado. Você pode incluí-los em sua exportação.',
    'do_export'                                  => 'Exportar',
    ...
    'instructions_rule_from_bill'                => 'Para conectar transações com sua nova fatura ":name", '.config('app.name').' pode criar uma regra que automaticamente será verificada a cada transação que você criar. Verifique os detalhes abaixo e salve a regra para que o '.config('app.name').' possa conectar automaticamente as transações a sua nova fatura.',
    ...
    'apply_rule_selection_intro'                 => 'As regras como ":title" normalmente são aplicadas apenas a transações novas ou atualizadas, mas você pode informar o '.config('app.name').' para executá-lo em uma seleção de suas transações existentes. Isso pode ser útil quando você atualizou uma regra e você precisa das alterações a serem aplicadas a todas as suas outras transações.',
    ...
    'apply_rule_group_selection_intro'           => 'Os grupos de regras como ":title" normalmente são aplicados apenas a transações novas ou atualizadas, mas você pode informar ao '.config('app.name').' para executar todas as regras neste grupo em uma seleção de suas transações existentes. Isso pode ser útil quando você atualizou um grupo de regras e você precisa das alterações a serem aplicadas a todas as suas outras transações.',
    ...
    'create_rule_for_bill_txt'                => 'Você acabou de criar uma nova fatura chamada ":name". Parabéns! O '.config('app.name').' pode combinar automagicamente novas retiradas com essa fatura. Por exemplo, sempre que você pagar seu aluguel, a fatura "aluguel" será vinculada à essa despesa. Dessa forma, '.config('app.name').' pode mostrar com precisão quais faturas são devidas e quais não são. Para isso, uma nova regra deve ser criada. O '.config('app.name').' preencheu alguns padrões coerentes para você. Por favor, verifique se estão corretos. Se estiverem corretos, o '.config('app.name').' irá vincular as retiradas à fatura correta automaticamente. Por favor, confira os gatilhos e adicione outros se estiverem errados.',
    ...
    'pref_languages_help'                     => config('app.name').' suporta muitos idiomas. Qual você prefere?',
    ...
    'delete_your_account_help'                => 'Excluindo sua conta também vai apagar quaisquer contas, transações, <em>qualquer coisa</em> que você pode ter salvo no '.config('app.name').'. Tudo será perdido.',
    ...
    'secure_pw_ff'                            => 'Você usa a mesma senha em toda a internet? Se um site perder sua senha, os hackers terão acesso a todos os seus dados. O '.config('app.name').' depende de você para escolher uma senha forte e única para proteger seus registros financeiros.',
    'secure_pw_check_box'                     => 'Para ajudá-lo o '.config('app.name').' pode verificar se a senha que você deseja usar foi roubada anteriormente. Se for esse o caso, o '.config('app.name').' aconselha a você NÃO utilizar essa senha.',
    'secure_pw_working_title'                 => 'Como isso funciona?',
    'secure_pw_working'                       => 'Ao marcar a caixa de seleção, o '.config('app.name').' enviará os cinco primeiros caracteres do hash SHA1 de sua senha <a href="https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/">ao site Troy Hunt</a>, para verificar se ela está na lista. Isso impede que você use senhas inseguras, como recomendado na última <a href="https://pages.nist.gov/800-63-3/sp800-63b.html">publicação NIST Special</a> sobre esse assunto.',
    ...
    'delete_local_info_only'                  => 'Como você se autentica por meio de ":login_provider", isso irá apagar apenas informações locais do '.config('app.name').'.',
    ...
    'currencies_intro'                        => config('app.name').' oferece suporte a várias moedas que você pode definir e ativar aqui.',
    ...
    'auto_match_on'                           => 'Automaticamente correspondente com '.config('app.name'),
    'auto_match_off'                          => 'Não corresponde automaticamente com '.config('app.name'),
    ...
    'reconcile_has_more'                      => 'Seu registro do '.config('app.name').' tem mais dinheiro nele do que o seu banco afirma que você deveria ter. Existem várias opções. Escolha o que fazer. Em seguida, pressione "Confirmar reconciliação".',
    'reconcile_has_less'                      => 'Seu registro do '.config('app.name').' tem menos dinheiro nele do que o seu banco afirma que você deveria ter. Existem várias opções. Escolha o que fazer. Em seguida, pressione "Confirmar reconciliação".',
    'reconcile_is_equal'                      => 'Seu registro do '.config('app.name').' e seus registros bancários combinam. Não há nada a se fazer. Pressione "Confirmar reconciliação" para confirmar sua entrada.',
    ...
    // new user:
    'welcome'                                 => 'Bem Vindo ao '.config('app.name').'!',
    'submit'                                  => 'Enviar',
    'submit_yes_really'                       => 'Enviar (eu sei o que estou fazendo)',
    'getting_started'                         => 'Iniciar',
    'to_get_started'                          => 'É bom ver que você instalou o '.config('app.name').' com sucesso. Para começar com esta ferramenta, insira o nome do banco e o saldo da sua principal conta corrente. Não se preocupe ainda se você tiver várias contas. Você pode adicionar aqueles mais tarde. É só que o '.config('app.name').' precisa de algo para começar.',
    'savings_balance_text'                    => 'O '.config('app.name').' criará automaticamente uma conta de poupança para você. Por padrão, não haverá dinheiro na sua conta de poupança, mas se você contar o saldo ao '.config('app.name').', ele será armazenado como tal.',
    'finish_up_new_user'                      => 'É isso aí! Você pode continuar pressionando <strong>Enviar</strong>. Você será levado ao índice de '.config('app.name').'.',
    'stored_new_accounts_new_user'            => 'Yay! Suas novas contas foram armazenadas.',
    'set_preferred_language'                  => 'Se você prefere utilizar o '.config('app.name').' em outro idioma, informe aqui.',
    'language'                                => 'Idioma',
    ...
    'import_index_title'                    => 'Importar transações para o '.config('app.name'),
    'import_data'                           => 'Importar dados',
    'import_transactions'                   => 'Importar transações',

    // sandstorm.io errors and messages:
    'sandstorm_not_available'               => 'Esta função não está disponível quando você está usando o '.config('app.name').' dentro de um ambiente Sandstorm.io.',
    ...
    'no_accounts_imperative_asset'          => 'Para começar a usar o '.config('app.name').', você deve criar pelo menos uma conta de ativos. Vamos fazer agora:',
    ...
    'no_recurring_intro_default'            => 'Você ainda não tem nenhuma transação recorrente. Você pode usá-las para que o '.config('app.name').' crie transações para você automaticamente.',
    ...
    'warning_duplicates_repetitions'        => 'Em raras ocasiões, as datas aparecem duas vezes na lista. Isso pode acontecer quando várias repetições colidem. '.config('app.name').' sempre irá gerar uma transação por dia.',
    ...
    'recurring_never_cron'                  => 'Parece que o cron job necessário para dar suporte a transações recorrentes nunca foi executado. Isso é normal quando você acabou de instalar o '.config('app.name').', mas deve ser configurado o quanto antes. Por favor, veja as páginas de ajuda usando o ícone (?) no canto superior direito da página.',
    ...
    'help_first_date_no_past'            => 'Indique a primeira recorrência esperada. '.config('app.name').' não irá criar transações no passado.',
    ...
    'help_weekend'                       => 'O que o '.config('app.name').' deve fazer quando a transação recorrente cai em um sábado ou domingo?',
